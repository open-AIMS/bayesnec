...
Any simple transformation may be written directly inside `crf`, as `crf(log(x), model = "nec4param")`. Two consequences follow, and a third applies to a transformation written inside `disp`. The transformation is evaluated before `bayesnec` inspects the predictor, so a control concentration recorded as `0` arrives at the data check as `log(0)` and the fit stops, reporting a predictor that is not finite. `bayesnec` does not correct a zero on the predictor axis, on either the recorded or the transformed scale: a concentration of zero is an ordinary control, and no response distribution constrains the values a predictor may take. An offset for a logarithm is therefore the user's to choose and to add to the data first. A logarithm written inside `disp`, as `disp(~log(x))`, fails differently and less clearly. That formula is evaluated by `brms` rather than by `bayesnec`, so nothing checks it: a zero concentration reaches Stan as `-Inf`, with a warning about infinite values in the data and no mention of the term responsible. And the toxicity estimates are then returned on the transformed scale, so `nec`, `ecx` and `nsec` need back-transforming, with `xform` doing the same job for plots:
