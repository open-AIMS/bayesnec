...
Important information on the current package is contained in the `bayesnec` help-files and the Model details [vignette][e2b].
...
```{r, warning = FALSE, eval=FALSE}
set.seed(333)
exp_3b <- bnec(data = count_data, x_var = "raw_x",
              y_var = "count", family = "negbinomial", model = "neclin")
```
The resultant plot seems to indicate the negative binomial family works better in terms of dispersion (more sensible wider confidence bands) however it appears there are some bi-modality in these parameter estimates, particularly in the position of the `nec` parameter, and an alternative model may be better for these data.
