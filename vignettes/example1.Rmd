...
Read in order. Substitution is not only wrong about the value, it is *confident*: the posterior SD is 0.014, an interval a twentieth the width of any other, because those rows have been asserted to sit exactly at the bound when all that is known is that they lie below it.

Left-censoring withdraws the false assertion, and what it reveals is that the data contain almost nothing about `bot`. The posterior widens by a factor of 160 and runs down to −11 d^-1^, a rate at which the population would fall by $e^{33}$ over the test. That is the prior's lower tail rather than an estimate. Nothing has gone wrong mechanically — the chains mix, $\widehat{R}$ is 1.00, and no divergences are reported. The number is simply uninformed, and an interval alone will not tell you so.

The third row is the repair. A population below the counting limit is not below extinction, so the truth lies in an interval and `cens(censoring, upper)` can say exactly that. Bounding the flat region from below restores a usable posterior. It is worth being clear about what that estimate then rests on: the lower end is the declared extinction floor, here 1 cell mL^-1^ and so −2.75 d^-1^, and the posterior presses against it. The asymptote is constrained by that assumption rather than measured, and should be reported that way.
