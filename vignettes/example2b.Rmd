...
A second exclusion applies to the 0, 1-bounded families (Binomial, Beta and Beta-Binomial) on an `"identity"` link. **nechormepwr** and **nechorme4pwr** include the hormesis term $x_i^{1/(1+e^{\alpha})}$, which has no coefficient: the term approaches 1 as $\alpha$ grows, at every concentration above zero, so for any $\tau$ above zero there is an $\alpha$ at which the mean exceeds 1, whatever the predictor range. The other hormesis equations --- **nechorme**, **nechorme4**, **nechormepwr01**, **ecxhormebc4** and **ecxhormebc5** --- are fitted on these families, because the increase prior to $\eta$ is scaled by a parameter the fit can shrink. `bayesnec`'s initial-value search starts them inside the support: `make_inits` draws from the priors, and a draw is accepted only where the whole initial curve falls within a band built from the replicated level means and their spread, intersected with the support of the mean.
...
The **nechormepwr01** model is a *hormesis* model [@Mattson2008], allowing an initial increase in the response variable at concentrations below $\eta$. The model is 0-bounded, thus not suitable for Gaussian response data or the use of a `"logit"` or `"log"` link function. In this case the linear version (**neclinhorme**) should be used. Its increase is bounded above at 1 by construction, which is what makes it the hormesis model for the 0, 1-bounded families and also why it is excluded from the 0, `Inf` bounded ones (Gamma, Poisson, Negative Binomial with an `"identity"` link), whose response may exceed 1.
