...
The model is `0` bounded, thus not suitable for gaussian response data or the use of a logit or log link function. In this case the linear version (**neclinhorme**) should be used. Because the model contains linear predictors it is not suitable for `0`, `1` bounded data (binomial and beta families with identity link).
...
$$y_i = (\tau + x_i^{1/(1+e^{alpha})}) e^{-\beta \left(x_i - \eta \right) f(x_i, \eta)}$$
and fitted in R via `brms` using the formula
```{r, echo=FALSE}
...
$$y_i = \delta + ((\tau + x_i^{1/(1+e^{alpha})}) - \delta ) e^{-\beta \left(x_i - \eta \right) f(x_i, \eta)}$$
and fitted in R via `brms` using the formula
```{r, echo=FALSE}
...
$$y_i = \left(\frac{1}{(1 + ((1/\tau)-1)  e^{-e^{\alpha}x_i}}\right) e^{-\beta \left(x_i - \eta \right) f(x_i, \eta)}$$
