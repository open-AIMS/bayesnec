...
There are a range of models available in `bayesnec` and the working `bnec` function supports individual model fitting, as well as multi-model fitting with Bayesian model averaging through. 
...
using the `loo_model_weights` from `loo` [@vehtari2020; @vehtari2017]. Individual `brms` model fits can be extracted from the `mod_fits` element and can be examined individually.

`model` may also be one of "all", meaning all of the available models will be fit; "ecx" meaning only models excluding the $\eta = \text{NEC}$ step parameter will be fit; "nec" meaning only models with the $\eta = \text{NEC}$ step parameter will be fit. There are a range of other pre-define model groups available. The full list of currently implemented model groups can be seen using
...
### ecxwb1
The **ecxwb1** model is a 4 parameter sigmoidal decay model which is a slight reformulate of the Weibull1 model of [@Ritz2016], given by the equation:
...
### ecxwb2
The **ecxwb2** model is a 4 parameter sigmoidal decay model which is a slight reformulate of the Weibull2 model of [@Ritz2016], given by the equation:
