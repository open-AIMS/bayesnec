...
Compilation is the other thing the arrangement adds. Every level fits the same equations, so parallel levels build the same Stan programs at the same time, and `cmdstanr` does not lock its compile cache. Each worker process is given a directory of its own to keep them apart, which means a parallel grouped run on a cold cache compiles each equation once per worker rather than once. The count above measures sampling only, so on short fits the arrangement it prefers can be the slower one on the clock. Where `cmdstanr_write_stan_file_dir` has been set, those directories are written under it as `bayesnec-stan-<pid>`, one per worker process per run; a process id does not repeat, so they are not read again and are safe to delete. Fitting the levels in sequence uses the cache as it always did.
...
Each level is given its own seed, realised in the calling session before the levels are dispatched and derived from `seed` where one was passed. A grouped call therefore gives the same estimates whichever arrangement it took, so the number of workers on the machine does not change an answer, and the realised seeds are kept on the returned object as `level_seeds`.
