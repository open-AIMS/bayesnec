...
`screen_models()` screens on those same three together, drops every candidate
that failed, and reports what went and why. That message is the record of the
exclusion, and is what a methods section cites; an exclusion step that leaves no
...
The sections below continue with `exp_5` rather than `exp_5_screened`, so that
the model set they describe is the one fitted above.
