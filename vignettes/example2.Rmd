...
In this case, because we have already fitted `"decline"` models, we can ignore the message regarding the missing *NEC* models---these are all models that are not appropriate for a `Beta` family with an `identity` link function, or allow hormesis, which we did not consider in this example.
