...
In our example, the chains are well mixed and our ACF plot looks good, so we can go ahead and interpret this model, although the brms warning message indicates that we might need more iterations. Let's add some more iterations to our model, and check the output again. Note that it is important replace the fit in our bayesnecfit object with the updated model.

```{r update-binomial-NEC, echo=FALSE}
out$fit <- update(out$fit, iter = 10000)
plot(out$fit)
