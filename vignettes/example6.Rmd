...
and Annex 5 is explicit about what not to do with values that fall outside the
expected range:
...
That sentence is written about negative *inhibition* — growth stimulation at low
concentrations — rather than about negative growth rates, so it is not a ruling
on this case. The reason it gives, however, is general, and it is the reason
here.

## What "distorts the error distribution" means

The phrase is precise rather than rhetorical. Write `µ(x)` for the fitted mean
at concentration `x`. For an observation whose true value is negative, the two
treatments contribute different things to the likelihood:

| treatment | contribution | what it asserts |
|---|---|---|
| floor to zero | `f(0; µ(x), σ)` | the true value **is** 0 — false |
| censor at zero | `F(0; µ(x), σ)` | the true value **is ≤ 0** — true |

As `µ(x)` falls, the censored contribution saturates at 1: a curve descending
further below zero is un-rewarded, but not penalised. The floored contribution
tends to 0 instead, so flooring exerts a **persistent upward force on the fitted
curve at exactly the concentrations where decline is strongest**.

This does not require a hurdle to bite. Flooring biases the fitted mean upward
in a plain regression too, because the likelihood goes on pulling `µ(x)` toward
the substituted value. It is a separate failure from the three above, and it
survives every attempt to work around them.
