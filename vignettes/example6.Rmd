...
This can be checked directly. Refitting B's survival component on
...
doses that appear in the file biases the threshold low, because the last dose
those records contain is one at which half the snails were still alive, so the
fit never sees the concentration at which survival collapses.
...
**The two components' estimates separate, and by widely varying amounts.** The
growth and survival no-effect estimates are close together for some
contaminants and orders of magnitude apart for others, with growth declining
from the bottom of the tested range in the latter while survival holds until
near the top. A single number from a survivors-only analysis cannot express
this, and the four tests would appear far more alike than they are.

**The combined endpoint separates from growth only at the higher effect
level.** The combined no-effect estimate equals the growth estimate in every
test, as the per-draw minimum requires, and the combined EC10 matches the
growth EC10 to the precision reported. The two diverge at EC50, where the
combined value is at or below the growth value in all four tests: by about 6%
for C, 3% for B and D, and not at all for A, whose growth EC50 of 1.1 sits
below the concentration at which its survival curve leaves the control. Note
that C and D separate at EC50 even though their survival no-effect estimates
sit two orders of magnitude higher: a model-averaged survival curve containing
smooth models is never perfectly flat, so it contributes a little decline well
below its nominal threshold. Differences in the final digit of the EC10 column
are at the scale of the predictor grid rather than anything interpretable.

**The survival estimates are the clearest gain.** These are the numbers a
survivors-only analysis cannot produce at any level of effect, and for
contaminant B in particular they are the more consequential of the two
endpoints. The combined endpoint contributes on the response scale, where it
gives the expected growth per individual exposed rather than per individual
that lived.
...
as average specific growth rate,
