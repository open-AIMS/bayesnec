...
`bnec_hurdle()` returns that quantity directly from its truncated growth fit.
...
used different lower-bound conventions in the fitted model and its
post-processing, so `bnec_hurdle()` refuses the route before fitting and points
to the corresponding joint hurdle family.
