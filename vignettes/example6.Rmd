...
Two properties of these models are easily misread, and both are stated here
before any code.

The survivors-only analysis is not an approximation. The hurdle log-likelihood
separates exactly into a term over all individuals and a term over the survivors
only,
...
Zero-inflated Gamma and Beta are the same model as the hurdle. Zero-inflation
differs from a hurdle only when the base distribution can itself produce zeros,
which neither the Gamma nor the Beta can. The Stan density `brms` generates for
`zero_inflated_beta` is the hurdle form, with no mixture at zero. `bayesnec`
treats the two identically and keeps the `brms` name, so `hurdle_gamma` and
`zero_inflated_beta` differ in name and in the support of the response, not in
structure.
...
# The factorised and joint implementations
...
The *factorised* route, `bnec_hurdle()`, fits two ordinary `bnec()` models: a
zero-bounded model to the non-zero subset and a `bernoulli` model to the
survival indicator over all individuals. It returns them together as a
`bayesnechurdlefit`, and combined predictions are formed afterwards by
multiplying the two posteriors.

The *joint* route, `bnec(family = "hurdle_gamma")`, fits one model with two
parameter blocks. `brms` names the second block `hu` (or `zi` for the
zero-inflated families), and `bayesnec` gives it its own concentration-response
curve with every non-linear parameter taking that prefix: `top` belongs to the
response block, `hutop` to the survival block.
...
### The consequences of placing the curve on survival
...
#>                 Q50 Q2.5 Q97.5
#> combined (NEC) 1.08 0.78  1.37
#> growth (NEC)   1.08 0.78  1.37
#> survival (NEC) 2.18 1.89  2.59
#> 
#> ECx estimates
#>                Q50 Q2.5 Q97.5
#> combined ec10 1.26 1.00  1.53
#> combined ec50 2.21 2.03  2.42
#> growth ec10   1.26 1.00  1.53
#> growth ec50   2.32 2.13  2.51
#> survival ec10 2.28 2.01  2.66
#> survival ec50 2.84 2.58  3.11
#> 
...
## The separation of the combined and growth endpoints
...
The same reasoning generalises, and determines where the combined endpoint is
informative. The combined curve is the product of the two components, and below
the survival
...
rbind(
  ...
  round(3)
#>                 Q50  Q2.5 Q97.5
#> growth NSEC   1.194 0.952 1.458
#> combined NSEC 1.225 0.971 1.488
#> growth EC10   1.265 0.996 1.529
#> combined EC10 1.265 0.996 1.529
#> growth EC50   2.319 2.128 2.507
#> combined EC50 2.212 2.035 2.416
```
...
The practical consequence is that a combined no-effect estimate is not on its
own a reason to fit a hurdle model, since it will generally reproduce the growth
one. The reasons to fit one are the survival curve, which a survivors-only
analysis cannot produce at all, and the effect sizes at concentrations where
mortality is operating, which such an analysis systematically understates.
...
#> top       24.76      0.72    23.40    26.29 1.00     6285     5306
#> beta      -0.58      0.09    -0.75    -0.40 1.00     4791     5399
#> nec        1.08      0.16     0.79     1.37 1.00     4507     4379
#> hutop      0.96      0.01     0.93     0.99 1.00     6902     4163
#> hubeta     0.09      0.20    -0.27     0.54 1.00     5793     5669
#> hunec      2.18      0.21     1.90     2.59 1.00     5723     6518
#> 
#> Further Distributional Parameters:
#>       Estimate Est.Error l-95% CI u-95% CI Rhat Bulk_ESS Tail_ESS
#> shape    11.03      1.23     8.77    13.65 1.00     8154     5124
#> 
...
not interchangeable, so check `class(fit)` where a component estimate comes back
looking like the combined one.
...
round(comparison, 3)
#>                             Q50  Q2.5 Q97.5
#> factorised, combined      1.075 0.783 1.365
#> joint, combined           1.081 0.793 1.366
#> factorised, growth EC10   1.265 0.996 1.529
#> joint, growth EC10        1.269 1.006 1.530
#> factorised, survival EC10 2.278 2.010 2.660
#> joint, survival EC10      2.278 2.014 2.658
```

Differences between the two routes are Monte Carlo noise rather than a
modelling choice.

## The recovery of the response block from survivors alone
...
#>                    Q50      Q2.5    Q97.5
#> survivors only 1.07513 0.7834951 1.365483
#> hurdle, growth 1.07513 0.7834951 1.365483
```
...
#> 1 0  24.77    0.964    23.83
#> 2 1  24.44    0.964    23.53
#> 3 2  14.80    0.961    14.14
#> 4 3   8.44    0.403     3.40
#> 5 4   4.81    0.132     0.64
#> 6 5   2.75    0.044     0.12
```
...
#>   nec3param     0.379     0.110     0.030      0  0.025  0.053
#>   nec4param     0.121     0.035     0.010      0  0.008  0.017
#>   ecx4param     0.030     0.009     0.002      0  0.002  0.004
#>   ecxexp        0.001     0.000     0.000      0  0.000  0.000
#>   ecxwb1        0.011     0.003     0.001      0  0.001  0.002
#>   ecxll3        0.092     0.027     0.007      0  0.006  0.013
```
...
#> No-effect toxicity estimates
#>                    Q50 Q2.5 Q97.5
#> combined (N(S)EC) 1.04 0.29  1.42
#> growth (N(S)EC)   1.04 0.31  1.42
#> survival (N(S)EC) 2.20 1.43  2.81
#> 
...
#> ECx estimates
#>                Q50 Q2.5 Q97.5
#> combined ec10 1.23 0.48  1.56
#> combined ec50 2.21 1.95  2.42
#> growth ec10   1.23 0.49  1.56
#> growth ec50   2.31 2.10  2.51
#> survival ec10 2.30 1.90  2.84
#> survival ec50 2.86 2.53  3.16
#> 
#> The combined endpoint is the expected response per individual exposed,
#> i.e. growth * survival. Use which = to select a component.
```
...
cannot express — `best_crossed()` reports the combination holding the highest
...
#> Regression Coefficients:
#>        Estimate Est.Error l-95% CI u-95% CI Rhat Bulk_ESS Tail_ESS
#> top       24.75      0.70    23.45    26.30 1.00     2426     1957
#> beta      -0.58      0.10    -0.75    -0.39 1.00     1592     2072
#> nec        1.08      0.16     0.80     1.37 1.00     1588     2116
#> hutop      0.96      0.01     0.93     0.99 1.00     2429     1563
#> hubeta     0.09      0.20    -0.28     0.51 1.00     1720     1710
#> hunec      2.18      0.20     1.89     2.59 1.00     1605     2057
#> 
#> Further Distributional Parameters:
#>       Estimate Est.Error l-95% CI u-95% CI Rhat Bulk_ESS Tail_ESS
#> shape    11.04      1.21     8.78    13.61 1.00     2692     2075
#> 
#> Draws were sampled using sample(hmc). For each parameter, Bulk_ESS
#> and Tail_ESS are effective sample size measures, and Rhat is the potential
#> scale reduction factor on split chains (at convergence, Rhat = 1).
...
# A worked chronic snail toxicity example
...
## The contribution of the reconstruction under contaminant B
...
#>               exposed              survived           doses_total doses_with_a_survivor 
#>                   284                    63                    14                     4
```
...
snails that hold that information are precisely the ones it discards. This is
...
### The effect of the reconstruction on the estimates

It changes the survival component only, and that is the point.
...
do.call(rbind, sens)
#>                              Q50       Q2.5      Q97.5
#> recorded deaths only  0.06596914 0.03669723 0.09693471
#> + first all-dead dose 0.08320376 0.05272932 0.09830386
#> + all all-dead doses  0.08480641 0.05635826 0.09806783
```

Two conclusions follow, and they have different implications.

Reinstating the first all-dead dose changes the survival threshold by about
25% and halves the credible interval. Analysing only the doses that appear in
the file biases that threshold low, because the last dose those records contain
is one at which half the snails were still alive, so the fit never sees the
concentration at which survival collapses. The growth estimate is unchanged,
which is why a survivors-only analysis cannot detect the loss.

The all-dead doses above it add about 3%, well inside the interval. Once the
fitted curve has reached zero, further doses at which every
...
The first decision concerns the four negative growth values. The assay is
destructive, so growth is obtained by referencing each snail to a baseline mean
rather than to its own starting size. That referencing has error in it, and four
survivors of 876 are slightly negative. The Gamma distribution cannot
accommodate them.
...
The second concerns the control on a log dose scale. Dose spans several orders
of magnitude, so the models are fitted on a log scale, and `log(0)` is
undefined. The control is
...
each page those workers touch stops being shared, so a large session raises the
memory every subsequent fit needs. None of the simulation fits are referenced
again.
...
the plots that follow. Holding all four six-model hurdle fits at once needs
more memory than anything else in this vignette, and is avoidable.
...
#> No-effect toxicity estimates
#>                    Q50 Q2.5 Q97.5
#> combined (N(S)EC) 0.59 0.15  1.12
#> growth (N(S)EC)   0.59 0.19  1.13
#> survival (N(S)EC) 1.69 1.10  2.40
#> 
#> NSEC values appear where a model set contains smooth (ECx) models, which
#> carry no threshold parameter; N(S)EC is a model-averaged combination
#> of the two (Fisher et al. 2023).
...
#> ECx estimates
#>                Q50 Q2.5 Q97.5
#> combined ec10 0.64 0.25  1.14
#> combined ec50 1.05 0.76  1.24
#> growth ec10   0.64 0.25  1.14
#> growth ec50   1.06 0.76  1.24
#> survival ec10 1.78 1.28  2.41
#> survival ec50 2.10 1.45  2.47
#> 
#> The combined endpoint is the expected response per individual exposed,
#> i.e. growth * survival. Use which = to select a component.
```
...
#> A.1           A   growth  0.590  0.640  1.100
#> A.2           A survival  1.700  1.800  2.100
#> A.3           A combined  0.590  0.640  1.100
#> B.1           B   growth  0.048  0.048  0.072
#> B.2           B survival  0.080  0.082  0.100
#> B.3           B combined  0.048  0.047  0.070
#> C.1           C   growth  0.028  0.030  3.500
#> C.2           C survival 14.000 15.000 16.000
#> C.3           C combined  0.028  0.030  3.500
#> D.1           D   growth  0.034  0.081  7.600
#> D.2           D survival 13.000 13.000 15.000
#> D.3           D combined  0.034  0.081  7.600
```
...
which is why B is the one that departs from growth at all, and by so little.
...
#>            survival
#> growth      nec3param nec4param ecx4param ecxexp ecxwb1 ecxll3
#>   nec3param     0.163     0.025     0.024      0  0.022  0.121
#>   nec4param     0.087     0.013     0.013      0  0.012  0.064
#>   ecx4param     0.056     0.009     0.008      0  0.008  0.042
#>   ecxexp        0.006     0.001     0.001      0  0.001  0.004
#>   ecxwb1        0.058     0.009     0.009      0  0.008  0.043
#>   ecxll3        0.089     0.014     0.013      0  0.012  0.066
```
...
## The conditions a hurdle Beta requires
...
#> No-effect toxicity estimates
#>                 Q50 Q2.5 Q97.5
#> combined (NEC) 1.68 1.32  1.91
#> growth (NEC)   1.69 1.41  1.91
#> survival (NEC) 2.96 1.44  3.95
#> 
#> ECx estimates
#>                Q50 Q2.5 Q97.5
#> combined ec10 1.98 1.72  2.19
#> combined ec50 3.41 2.99  3.77
#> growth ec10   1.99 1.75  2.19
#> growth ec50   3.69 3.44  3.99
#> survival ec10 3.47 2.19  4.37
#> survival ec50 6.30 5.21  8.12
#> 
#> The combined endpoint is the expected response per individual exposed,
#> i.e. growth * survival. Use which = to select a component.
```
...
#> Regression Coefficients:
#>        Estimate Est.Error l-95% CI u-95% CI Rhat Bulk_ESS Tail_ESS
#> top        0.68      0.01     0.66     0.71 1.00     5533     5155
#> beta      -1.07      0.09    -1.25    -0.90 1.00     4987     4738
#> nec        1.69      0.13     1.40     1.91 1.00     4326     3697
#> zitop      0.97      0.02     0.91     0.99 1.00     6661     4402
#> zibeta    -1.58      0.26    -2.15    -1.10 1.00     4056     3824
#> zinec      2.96      0.52     1.34     3.95 1.00     3966     2926
#> 
#> Further Distributional Parameters:
#>     Estimate Est.Error l-95% CI u-95% CI Rhat Bulk_ESS Tail_ESS
#> phi    28.41      4.11    21.09    37.47 1.00     5893     5081
#> 
#> Draws were sampled using sample(hmc). For each parameter, Bulk_ESS
#> and Tail_ESS are effective sample size measures, and Rhat is the potential
#> scale reduction factor on split chains (at convergence, Rhat = 1).
...
rbind(factorised = nec(zib_f),
      joint      = nec(zib_j))
#>                 Q50     Q2.5    Q97.5
#> factorised 1.681525 1.318349 1.906091
#> joint      1.675964 1.271652 1.906585
```
...
the snail example below, where reinstating a single missing dose changes the
...
# A counter-example with zeros that are not structural
...
## The kinds of zero a response can hold
...
## The meaning of "distorts the error distribution"
...
1. *Do not floor.* Model on a scale that admits the values as measured — a
specific growth rate, or the raw increment — using a family on the real line
with an equation that has a lower asymptote (`nec4param`, `ecx4param`) so the
curve can cross zero. `bayesnec` already drops the zero-bounded equations for
Gaussian data. Drive any hurdle from the recorded failure indicator, not from
`y == 0`.

2. *Where the shape of the curve below zero is not of interest, censor there.*
...
What is given up is that censored observations retain only near-quantal information: the
...
rate and a large one become close to indistinguishable, so the distinction
between growth arrest and active loss of biomass is largely given up. Where that
...
response block. Collapsing the two columns into one is flooring by another
route.

`bayesnecformula` passes the `cens()` aterm through to the fit, so this route
is available directly; see the Censoring section of `vignette("example1")` for
the declaration and for what a censored likelihood can and cannot recover.

3. *Where the values are already floored and unrecoverable*, the same
left-censored likelihood is the salvage: it replaces a false assertion with a
true one, which is the most that can be recovered. This is a repair, whereas
item 2 is a design.

4. *Where a strictly positive scale is required*, model the response relative to
...
the concentration that holds the most information about the survival curve.
