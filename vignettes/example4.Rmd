...
# Fit a set of models
exmp <- bnec(y ~ crf(x, model = "decline"), data = dat, iter = 2e3, refresh = 0)
```
...
This call fits all models that are suitable for modelling Gaussian-distributed response data, excluding all of the hormesis models, which we are not considering here. We have reduced the iterations back to the `brms` default to make the example lighter, but we do not recommend this in practice. Now that we have our example fit, we can use this to make different model sets, purely for the purposes of illustrating `compare_posterior` in `bayesnec` and highlighting the rich information that is contained in the Bayesian posterior draws.
...
<img src="vignette-fig-exmp4-posterior-1.png" title="plot of chunk exmp4-posterior" alt="plot of chunk exmp4-posterior" width="60%" style="display: block; margin: auto;" />
...
#> 1   all 0.0000000 0.9540945 0.9062906 1.004799
#> 2   all 0.2040816 0.9539804 0.9062892 1.004749
#> 3   all 0.4081633 0.9538954 0.9062868 1.004641
#> 4   all 0.6122449 0.9538954 0.9062827 1.004322
#> 5   all 0.8163265 0.9538770 0.9062759 1.003885
#> 6   all 1.0204082 0.9538442 0.9062644 1.003274
head(post_comp_fitted$diff_data)
#>   comparison diff.Estimate   diff.Q2.5 diff.Q97.5         x
#> 1    all-ecx  -0.003432207 -0.07204591 0.06586498 0.0000000
#> 2    all-ecx  -0.003416028 -0.07204438 0.06583873 0.2040816
#> 3    all-ecx  -0.003388660 -0.07204222 0.06580039 0.4081633
#> 4    all-ecx  -0.003485200 -0.07203920 0.06574442 0.6122449
#> 5    all-ecx  -0.003449763 -0.07203495 0.06549196 0.8163265
#> 6    all-ecx  -0.003432207 -0.07202900 0.06472865 1.0204082
```
...
<img src="vignette-fig-exmp4-fitted-1.png" title="plot of chunk exmp4-fitted" alt="plot of chunk exmp4-fitted" width="60%" style="display: block; margin: auto;" />
...
<img src="vignette-fig-exmp4-fitteddiff-1.png" title="plot of chunk exmp4-fitteddiff" alt="plot of chunk exmp4-fitteddiff" width="60%" style="display: block; margin: auto;" />
...
<img src="vignette-fig-exmp4-probplot-1.png" title="plot of chunk exmp4-probplot" alt="plot of chunk exmp4-probplot" width="60%" style="display: block; margin: auto;" />
