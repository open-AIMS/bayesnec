---
title: "Negative response values"
output:
  rmarkdown::html_vignette:
vignette: >
  %\VignetteIndexEntry{Negative response values}
...
plot. It is therefore common practice to remove it, either by replacing the
negative values with zero before fitting, or by constraining the fitted curve so
that its lower asymptote cannot fall below zero, or both.
...
is necessary to be precise about where the reported endpoints sit relative to
that boundary. Two endpoints are used throughout: an ECx and the NSEC.

## The ErCx notation

The subscript `r` in ErC50 is not a `bayesnec` convention; it comes from OECD
Test Guideline 201 [@oecd2026tg201]. An algal growth inhibition test yields two
distinct response variables from the same cell counts, and the guideline
distinguishes the endpoints derived from them by subscript:

* **ErC50** is based on the average specific **growth rate** — the rate constant
  of population increase, with units of reciprocal time.
* **EyC50** is based on **yield** — the biomass increase over the test, that is,
  final density minus initial density.
...
Note that `ecx(type = "absolute")` requires a model with a `bot` parameter when
the family is Gaussian; `bayesnec` will stop with an informative error otherwise,
because without a lower asymptote there is nothing to anchor the absolute scale.
...
finite level of noise. Section \@ref(nsec-results) returns to this, because it
determines how the NSEC results have to be read.

# The model, and why only one
...
model set were free to change at the same time, a shift in the endpoint could
come from either. Holding the functional form fixed makes the comparison
attributable. `nec4param` is the natural choice because it has an explicit lower
asymptote `bot`, which is the parameter the conventions act on.
...
`nec3param` is the three-parameter NEC model, which is `nec4param` with `bot`
fixed at zero. It is precisely the model an analyst reaches for when they want
the curve to stop at zero — and it is dropped here, and would in any case block
`ecx(type = "absolute")` because it has no `bot` parameter. This is why the
zero-constrained approaches below are built from a *prior* on `bot` rather than
by switching model.
...
# Six ways of handling the negative values

Six approaches are compared. Three retain the measured values; three impose the
zero boundary. They are labelled A to D below for compactness, and the labels
are used consistently in the results.

Every approach uses the same model, the same family and — importantly — the
*same prior*. `bnec()` derives its default Gaussian priors from the response
...
Real datasets can show that the six approaches disagree, but not which of them
...
one, and it is a misspecification shared equally by all six approaches.
...
ggplot(point_data, aes(x = x, y = y)) +
  ...
  geom_line(data = curve_data, aes(y = mu), linewidth = 0.7) +
  facet_wrap(~ scenario, scales = "free", ncol = 3,
             labeller = labeller(scenario = lab)) +
  labs(x = "Concentration", y = expression("Specific growth rate ("*d^-1*")")) +
...
## What each approach does to the fitted curve


``` r
# One colour per approach, used consistently in every figure below. The three
# approaches that retain the measurement are given cool-to-warm hues distinct
# from the three that impose the boundary, and the panels separate them as well,
# so the figures do not rely on colour alone.
arm_cols <- c(A = "#0E7C61", C = "#C25E12", D = "#8E44C9",
              B1 = "#1B6BC4", B3 = "#B0217A", B2 = "#7C7A00")
arm_levels <- c("A", "C", "D", "B1", "B3", "B2")
```


Before the aggregate results, it is worth looking at a single simulated dataset
fitted by all six approaches, because the summary statistics are easier to
...
``` r
# posterior means from the six fits to one simulated dataset from scenario 8
example_pars <- read.csv(text = "
arm,bot,top,beta,nec,sigma
A,-0.43045,0.11839,-3.28625,1.93283,0.02249
C,-0.03579,0.11865,-1.67881,1.97273,0.01332
D,-0.08428,0.11860,-1.92347,1.95741,0.01321
B1,-0.00280,0.11851,-1.20377,2.18407,0.01225
B3,0.00000,0.11841,-1.13271,2.23954,0.01219
B2,0.00000,0.11709,-0.28537,2.93709,0.04703
")
...
# the actual dataset these six models were fitted to, rather than a fresh draw,
...
ggplot(ex_curves, aes(x = x, y = y)) +
  geom_hline(yintercept = 0, linetype = "dashed", colour = "grey50") +
  geom_vline(xintercept = example_truth$ErC50, linetype = "dotted",
             colour = "grey40") +
  geom_point(data = ex_dat, aes(shape = y < 0), alpha = 0.5, size = 1) +
  geom_line(data = true_curve, linewidth = 1.1, colour = "grey15") +
...
<div class="figure" style="text-align: center">
<img src="vignette-fig-example-curves-1.png" alt="Fitted curves from all six approaches applied to a single simulated dataset from scenario 8, with the true curve in black. Points are the simulated observations; open points are the nine negative values that the conventions act on. The vertical line is the true ErC50." width="90%" />
<p class="caption">Fitted curves from all six approaches applied to a single simulated dataset from scenario 8, with the true curve in black. Points are the simulated observations; open points are the nine negative values that the conventions act on. The vertical line is the true ErC50.</p>
</div>
...
curve in the region the endpoints *are* read from. `nec` moves from 1.93 under
...
endpoint section: the constraint applies below zero, but its consequences are
felt above it.
...
Table: Posterior mean parameters from the six fits to one simulated dataset. True values are bot = -0.476 and nec = 1.3.
...
One caution about this figure: it is one dataset, and the individual endpoint
differences are within Monte Carlo noise. It is included to show the *shapes*
...
six approaches, giving 12 x 240 x 6 = 17,280 model fits. Sampling used four
...
divergent transitions per fit is 0.00 for A, 0.03 for B3, 0.06 for B1, 0.07 for
C, 1.21 for D and **5.67 for B2**; mean maximum R-hat is between 1.004 and 1.012
throughout. B2's sampling difficulty is a symptom of the same problem as its
inflated intervals: it is being asked to fit a curve that cannot reach its own
observations.
...
arm,endpoint,regime,rel_bias,coverage,rmse
...
## The result depends on the design
...
approach: there are no data in the region that determines it, and all six carry a
shared bias of similar size and the same sign. Where the series runs past the
...
In the *stops short* column the six approaches are within about four percentage
points of each other and B1 has the smallest bias of any. It would be a mistake
...
cent for ErC50; truncation (D) is close behind; and the three approaches that
impose the zero boundary are biased low by 5 to 9 per cent, in the direction of
making the substance appear more toxic than it is.
...
-7.4% to -7.6%, and B2 from -8.2% to -10.0% to -9.8%, while A and C stay inside
±3.6% with no trend. This is the mechanism working as expected: the further the
data descend below the boundary, the more the boundary distorts.

ErC10 behaves differently and needs care. At `R` = 2.3 every approach is biased
*high* on ErC10, by between 15% and 57%, and the intact analysis is fifth of six.
That is not a reversal of the ordering; it is a shared estimation difficulty
swamping the differences between approaches. ErC10 requires a 10% decline from
the control to be resolved against the residual scatter, and at a 9.6% control
coefficient of variation that is a demanding thing to ask. The next section
resolves it.

## Measurement precision separates bias from estimation error
...
arm,endpoint,R,cv,rel_bias,coverage
...
")
precision$handling <- ifelse(precision$arm %in% c("A", "C", "D"),
                             "Measurement retained", "Zero boundary imposed")
...
ggplot(precision, aes(x = cv_f, y = 100 * rel_bias, colour = arm, group = arm)) +
  ...
  geom_point(size = 1.8) +
  facet_grid(endpoint ~ handling, scales = "free_y") +
...
The two panels on the left show every retained-measurement approach converging on
the true value: arm A's ErC10 bias falls from +19.0% to +1.4% and arm C's from
+11.1% to +1.0%, and both ErC50 biases sit within one per cent throughout. The
panels on the right show the three boundary-imposing approaches levelling off:
B1 at +13.2% and B2 at +49.6% on ErC10, and at -8.2% and -11.3% on ErC50. A
...
ordering, and the intact analysis is no longer fifth of six. The apparent
...
At the highest precision tested, B1 and B3 contained the true ErC50 in **none**
of the 240 simulated datasets, and B3 likewise for ErC10. The practical reading
is uncomfortable: a laboratory that improves its technique, adds replicates and
reduces its control variability will, if it floors its negative values, produce
an analysis that is more confident and no less wrong.
...
The NSEC needs separate treatment, and it turns out to be the endpoint most
strongly affected. Because the NSEC is often the value actually reported, this
matters.

### Why the NSEC cannot be scored at a single noise level
...
B2,1.9,3.037,2.132
")
nsec_precision$handling <- ifelse(nsec_precision$arm %in% c("A", "C", "D"),
                                  "Measurement retained", "Zero boundary imposed")
...
**47%, 53% and 134%**. These are not estimation error and they do not improve
with a better experiment.
...
* **ErC50 is biased low** under all three conventions, by 5 to 11%. The substance
  appears *more* toxic than it is.
* **The NSEC is biased high**, by 47 to 134%. The substance appears *less* toxic
  than it is.
...
### A sign change worth understanding
...
precise one. Both directions are consequences of the two mechanisms described in
...
which is why it is above A at every level and by a growing margin.

This is why the full set of six was fitted rather than the intact analysis
...
# The same comparison on real data
...
Four marine microalgal growth tests were analysed by all six approaches. Two are
...
dataset,arm,endpoint,r,rlo,rhi,usable
...
perfect coverage with an 11%-low point estimate and enormous intervals; B1 and B3
achieve narrow intervals with zero coverage. Either statistic on its own
identifies the wrong approach as the best one.
...
of prior scale — and comparing how far the plateau and the reported endpoints
moved.
...
arm,endpoint,tighter,default,wider,endpoint_span_pct,bot_tighter,bot_default,bot_wider,bot_span_x
...
Table: How far each reported endpoint moves across the same sixteen-fold change in the bot prior, as a percentage of its value under the default prior.
...
**The reported endpoints, however, are robust in every approach.** The largest
...
chose deliberately. This is the useful form of the result: **the endpoints can be
...
is a real misspecification, shared equally by all six approaches, and it is
...
**Only one model form was fitted.** Holding the model at `nec4param` is what
makes the comparison attributable to the data-handling choice, but it means the
results do not directly describe what happens under `bayesnec`'s usual
model-averaged workflow. The zero boundary acts on the lower asymptote, so the
qualitative conclusion should carry to any model with a free lower asymptote;
