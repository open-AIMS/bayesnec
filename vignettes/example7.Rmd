...
conventions can be scored rather than merely compared; whether fitting the
package's candidate set instead of one equation changes the answer; and what the
same comparison looks like on real data.
...
Two of those defaults matter and they are separable. Not flooring is the one
that carries the argument. Fitting a candidate set rather than one equation
turns out to matter almost as much once the data *have* been floored: it
recovers most of the lost accuracy on the ErC50, and on the NSEC it helps at
realistic noise and hurts at high precision. Both are quantified below.
...
The consequence for the present problem is as follows. On the
...
Three aspects of that specification require comment.

**The functional form is held fixed.** `bayesnec` ordinarily fits a candidate
set and averages it (see the [Multi model usage][e2] vignette), and that remains
the recommendation for estimating a toxicity value. It is set aside for the main
comparison because the object of study is a *data-handling choice*: were the
model set free to vary at the same time, a shift in the estimate could be
attributed to either. Fixing the form also removes misspecification from the
simulation, since `nec4param` is the generating model. The *Model averaging*
section relaxes this and reports the consequences.

**The model carries an explicit lower asymptote.** `bot` is the parameter two
...
**The family determines the candidate set.** Under a Gaussian family
...
**Models fitted.** For the six Gaussian approaches, across all twelve
simulation scenarios and all four case studies, the model is `nec4param` with
`family = gaussian(link = "identity")`. The two family-floored approaches use
`nec3param` with a Beta or a Gamma: `nec3param` holds the lower asymptote at
zero, which makes them the family-enforced counterpart of B2 and B3. Recorded
explicitly because the rules governing which models are valid for a given family
are expected to change (issue #195), so a reader reproducing this work should
fix the model rather than rely on the default set of the day.

**The link is the identity.** `bnec()` sets `link = "identity"` for every family
...
`get_priors()`, and pass that object to every subsequent fit. `get_priors()` is
the accessor to use here, not `pull_prior()`: the latter returns the whole
`brmsprior` a fit carries --- `brms` defaults and duplicated vectorized rows
included, wrapped in a list --- and is documented as not accepted by
`bnec(prior = )`. Passing it is not currently an error, so a prior modified that
way is silently discarded and the fit quietly falls back to its own defaults.

### Approaches retaining the measurement
...
shared_prior <- get_priors(fit_A)
shared_prior
#>                     prior class coef group resp dpar nlpar     lb    ub
#>              normal(0, 5)     b                       beta   <NA>  <NA>
#>  normal(-0.270, 0.404...)     b                        bot   <NA>  <NA>
#>               gamma(5, 8)     b                        nec  0.018  18.5
#>   normal(0.128, 0.404...)     b                        top   <NA>  <NA>
```
...
### Approaches imposing the boundary directly
...
`bnec()` accepts a `constant()` prior directly: the fixed value is assigned
rather than sampled during the initial-value search, and dropped again before
the model is handed to Stan, which does not declare a parameter whose prior is
constant. No initial values need to be supplied by hand (#244).
...
### Approaches imposing the boundary through the family
...
pre-empted, since it is part of the convention being examined. Under a Gamma
`bnec()` reports the shift with a message; under a Beta it does not, so in
approach E --- the most common preparation of the four --- the last step of the
flooring happens silently.
...
The simulation is not run by this vignette. It is a separate research
compendium, [open-AIMS/negative-sgr](https://github.com/open-AIMS/negative-sgr)
[@negativesgr2026], which carries the generating code, the `targets` pipeline
that runs it, an `renv` lockfile pinning every package version, and the result
tables. Fitting it takes days of machine time on many cores, so it cannot run
at vignette build time.

**Every figure quoted below is transcribed from that compendium's output at a
fixed commit**, `0181d66e`, and appears here as a literal rather than as
something recomputed. The tables you can regenerate and check against this
document are `analysis/phase5_metrics.csv` for the eight-approach comparison
and `analysis/phase10_metrics.csv` for the model-averaging section, with
`analysis/phase10_vignette_numbers.R` producing the summary this vignette
quotes.

Two consequences follow, neither visible from the code shown here. The
reported figures are **frozen**: they describe `bayesnec`
as it behaved at that commit, and nothing in this package's checks will notice
if later changes move them. And the compendium, not this vignette, is the
citable record of the study --- the description below is a summary of its
design, complete enough to follow the argument but not a substitute for it.
...
# chosen by eye. Two consequences constrain how the figures may be drawn. The legend order is not arbitrary: the obvious
...
# The simulated dataset these eight models were fitted to, rather than a fresh
# draw, so that the points shown are the points the curves were estimated from.
# Held here as the intact measurements; each panel below is given the version its
# own approach saw.
...
# Each panel shows the data its own fit was given, not one intact copy repeated
# eight times: B1, B3, E and F were handed floored values, C the bound with the
# row declared censored, and D a series truncated at the concentration where
# approach A's own posterior mean first reaches zero -- which is the rule the
# approach is defined by, recovered here from A's stored parameters rather than
# hard-coded. Only A and B2 saw the measurements as simulated. Open symbols mark
# the rows the convention altered.
ex_x <- sort(unique(ex_dat$x[ex_dat$x > 0]))
ex_A <- example_pars[example_pars$arm == "A", ]
ex_truncation <- min(ex_x[nec4param_curve(ex_x, ex_A$top, ex_A$bot,
                                          ex_A$beta, ex_A$nec) <= 0])

ex_points <- do.call(rbind, lapply(arm_levels, function(a) {
  d <- subset(ex_dat, x > 0)
  d$altered <- FALSE
  if (a %in% c("B1", "B3", "E", "F", "C")) {
    d$altered <- d$y < 0
    d$y <- pmax(d$y, 0)
  } else if (a == "D") {
    d <- d[d$x < ex_truncation, ]
  }
  data.frame(arm = a, d)
}))
ex_points$arm <- factor(ex_points$arm, levels = arm_levels)

ggplot(subset(ex_curves, x > 0), aes(x = x, y = y)) +
  geom_hline(yintercept = 0, linetype = "dashed", colour = "grey50") +
  geom_vline(xintercept = example_truth$ErC50, linetype = "dotted",
             colour = "grey40") +
  geom_point(data = ex_points, aes(shape = altered), alpha = 0.5, size = 1) +
...
<div class="figure" style="text-align: center">
<img src="vignette-fig-example-curves-1.png" alt="Fitted curves from all eight approaches applied to a single simulated dataset from scenario 8, with the true curve in black. Each panel shows the data that approach was given: intact under A and B2, floored to zero under B1, B3, E and F, the censoring bound under C, and truncated above 12.2 under D. Open symbols mark the nine rows the convention altered. Curves are drawn from posterior mean parameters, which for a non-linear model is not the same as the posterior mean curve; the difference is largest for B2. The vertical line is the true ErC50." width="90%" />
<p class="caption">Fitted curves from all eight approaches applied to a single simulated dataset from scenario 8, with the true curve in black. Each panel shows the data that approach was given: intact under A and B2, floored to zero under B1, B3, E and F, the censoring bound under C, and truncated above 12.2 under D. Open symbols mark the nine rows the convention altered. Curves are drawn from posterior mean parameters, which for a non-linear model is not the same as the posterior mean curve; the difference is largest for B2. The vertical line is the true ErC50.</p>
</div>
...
to F's average. Across the 500 datasets of this scenario F's mean ErC50 bias is
...
and the mechanism. The aggregate results below, each averaging 500 simulated
...
eight approaches, giving 12 x 500 x 8 = 48,000 model fits. The iteration count
was set from the Monte Carlo standard error on coverage, which is the least
precise of the reported quantities: coverage is a proportion, so 500 iterations
give a standard error of about 1.0 percentage point at a true coverage of 0.95
and about 2.2 points near 0.5. That is far smaller than the differences the study
reports, which run from several percentage points to the difference between 0.95
and 0.00. Sampling used four
...
divergent transitions per fit is 0.00 for A and E, 0.01 for F, 0.03 for B3,
0.04 for B1, 0.05 for C, 1.04 for D and **5.81 for B2**; mean maximum R-hat is
between 1.004 and 1.012 throughout.
...
A,ErC10,reaches,0.2409,0.8380,0.6399
C,ErC10,reaches,0.1432,0.8873,0.4502
D,ErC10,reaches,0.1620,0.8833,0.5063
B2,ErC10,reaches,0.5634,0.8540,1.3115
B3,ErC10,reaches,0.2260,0.6793,0.6472
B1,ErC10,reaches,0.1939,0.7587,0.5836
E,ErC10,reaches,0.1262,0.9193,0.4110
F,ErC10,reaches,0.5459,0.6927,1.6560
A,ErC10,stops short,0.0799,0.9370,0.3872
C,ErC10,stops short,0.0679,0.9369,0.3635
D,ErC10,stops short,0.0705,0.9430,0.3678
B2,ErC10,stops short,0.1337,0.8617,0.5200
B3,ErC10,stops short,0.0956,0.8853,0.4225
B1,ErC10,stops short,0.0602,0.9377,0.3452
E,ErC10,stops short,0.1029,0.9037,0.4769
F,ErC10,stops short,0.2692,0.8430,0.9872
A,ErC50,reaches,0.0057,0.9293,0.3814
C,ErC50,reaches,-0.0025,0.8447,0.4601
D,ErC50,reaches,-0.0172,0.8500,0.4757
B2,ErC50,reaches,-0.0950,0.9700,0.6302
B3,ErC50,reaches,-0.0652,0.6167,0.5261
B1,ErC50,reaches,-0.0492,0.7053,0.5206
E,ErC50,reaches,-0.0869,0.6407,0.5911
F,ErC50,reaches,-0.0164,0.7700,1.1113
A,ErC50,stops short,-0.0457,0.7500,0.5156
C,ErC50,stops short,-0.0368,0.7853,0.4994
D,ErC50,stops short,-0.0391,0.7910,0.5132
B2,ErC50,stops short,-0.0745,0.6163,0.6355
B3,ErC50,stops short,-0.0534,0.7033,0.5503
B1,ErC50,stops short,-0.0303,0.8140,0.4951
E,ErC50,stops short,-0.1040,0.5493,0.7744
F,ErC50,stops short,-0.0488,0.7373,0.6874
")
```
...
approach: at `top_factor` = 0.8 no true mean reaches zero at all and at 1.0
only the highest concentration does, so there is essentially nothing in the
region that determines the asymptote, and all eight
...
|:--------|--------------:|------------------:|------------------:|----------------------:|
|A        |          0.006|             -0.046|              0.929|                  0.750|
|C        |         -0.002|             -0.037|              0.845|                  0.785|
|D        |         -0.017|             -0.039|              0.850|                  0.791|
|B2       |         -0.095|             -0.074|              0.970|                  0.616|
|B3       |         -0.065|             -0.053|              0.617|                  0.703|
|B1       |         -0.049|             -0.030|              0.705|                  0.814|
|E        |         -0.087|             -0.104|              0.641|                  0.549|
|F        |         -0.016|             -0.049|              0.770|                  0.737|
...
-10.4%, stands outside that band. It would be a mistake
...
the zero boundary deliberately are biased low by 5 to 10 per cent (B1 -4.9%,
B3 -6.5%, B2 -9.5%), in the
direction of making the substance appear more toxic than it is. The two
approaches where the *family* imposes the boundary are E at -8.7%, worse than any
of the deliberate ones bar B2, and F at -1.6%.

**F's -1.6% must not be read as accuracy.** It is a mean over three `delta`
levels of a quantity that changes sign across them, and its RMSE is 1.11 —
nearly three times arm A's 0.38 and the largest of any approach. An estimate
that is 14% high in one scenario and 13% low in another averages to something
respectable while being wrong in both.

The bias also deepens as the true lower asymptote gets further below zero. Across
`delta` = 2, 4 and 8, B1 moves from -2.8% to -5.6% to -6.3%, B3 from -4.3% to
-7.6% to -7.6%, B2 from -8.5% to -10.2% to -9.9%, and E from -5.6% to -10.6% to
-9.9%, while A and C stay inside ±3.4% with no trend. This is the mechanism
...
approach that crosses zero on the way: **+14.0%, then -5.6%, then -13.4%**.

ErC10 behaves differently and needs care. At `R` = 2.3 every approach is biased
*high* on ErC10, by between 13% and 56%, and the intact analysis is sixth of
...
A,ErC10,2.3,9.6,0.1906,0.894
A,ErC10,3.3,6.7,0.1207,0.918
A,ErC10,17,2.8,0.0345,0.964
A,ErC10,73,1.9,0.0145,0.982
C,ErC10,2.3,9.6,0.1044,0.920
C,ErC10,3.3,6.7,0.0675,0.926
C,ErC10,17,2.8,0.0234,0.930
C,ErC10,73,1.9,0.0096,0.958
D,ErC10,2.3,9.6,0.1257,0.894
D,ErC10,3.3,6.7,0.0758,0.916
D,ErC10,17,2.8,0.0238,0.928
D,ErC10,73,1.9,0.0096,0.950
B2,ErC10,2.3,9.6,0.5396,0.896
B2,ErC10,3.3,6.7,0.5154,0.906
B2,ErC10,17,2.8,0.4979,0.980
B2,ErC10,73,1.9,0.4954,0.996
B3,ErC10,2.3,9.6,0.2137,0.698
B3,ErC10,3.3,6.7,0.1990,0.546
B3,ErC10,17,2.8,0.1567,0.034
B3,ErC10,73,1.9,0.1491,0.000
B1,ErC10,2.3,9.6,0.1683,0.782
B1,ErC10,3.3,6.7,0.1502,0.718
B1,ErC10,17,2.8,0.1310,0.180
B1,ErC10,73,1.9,0.1310,0.018
E,ErC10,2.3,9.6,0.1074,0.928
E,ErC10,3.3,6.7,0.1246,0.874
E,ErC10,17,2.8,0.1264,0.410
E,ErC10,73,1.9,0.1242,0.126
F,ErC10,2.3,9.6,0.4926,0.678
F,ErC10,3.3,6.7,0.3946,0.668
F,ErC10,17,2.8,0.2383,0.916
F,ErC10,73,1.9,0.1574,0.988
A,ErC50,2.3,9.6,-0.0040,0.948
A,ErC50,3.3,6.7,-0.0087,0.938
A,ErC50,17,2.8,-0.0060,0.900
A,ErC50,73,1.9,-0.0038,0.888
C,ErC50,2.3,9.6,-0.0066,0.850
C,ErC50,3.3,6.7,-0.0145,0.824
C,ErC50,17,2.8,-0.0130,0.790
C,ErC50,73,1.9,-0.0095,0.768
D,ErC50,2.3,9.6,-0.0205,0.840
D,ErC50,3.3,6.7,-0.0202,0.818
D,ErC50,17,2.8,-0.0132,0.794
D,ErC50,73,1.9,-0.0095,0.794
B2,ErC50,2.3,9.6,-0.1018,0.970
B2,ErC50,3.3,6.7,-0.1075,0.994
B2,ErC50,17,2.8,-0.1129,1.000
B2,ErC50,73,1.9,-0.1135,1.000
B3,ErC50,2.3,9.6,-0.0758,0.598
B3,ErC50,3.3,6.7,-0.0888,0.338
B3,ErC50,17,2.8,-0.1007,0.004
B3,ErC50,73,1.9,-0.1019,0.000
B1,ErC50,2.3,9.6,-0.0562,0.708
B1,ErC50,3.3,6.7,-0.0704,0.512
B1,ErC50,17,2.8,-0.0814,0.022
B1,ErC50,73,1.9,-0.0817,0.002
E,ErC50,2.3,9.6,-0.1059,0.584
E,ErC50,3.3,6.7,-0.1321,0.236
E,ErC50,17,2.8,-0.1504,0.000
E,ErC50,73,1.9,-0.1482,0.000
F,ErC50,2.3,9.6,-0.0556,0.892
F,ErC50,3.3,6.7,-0.1059,0.800
F,ErC50,17,2.8,-0.1475,0.374
F,ErC50,73,1.9,-0.1562,0.116
")
...
<div class="figure" style="text-align: center">
<img src="vignette-fig-precision-bias-1.png" alt="Relative bias in the estimated ErC10 and ErC50 against the control coefficient of variation, which decreases to the right as the experiment becomes more precise. Each point summarises 500 simulated datasets. Approaches that retain the measurement converge on zero bias; approaches that impose the zero boundary converge on a non-zero value." width="95%" />
<p class="caption">Relative bias in the estimated ErC10 and ErC50 against the control coefficient of variation, which decreases to the right as the experiment becomes more precise. Each point summarises 500 simulated datasets. Approaches that retain the measurement converge on zero bias; approaches that impose the zero boundary converge on a non-zero value.</p>
</div>

The left-hand panels show every retained-measurement approach converging on the
true value: arm A's ErC10 bias falls from +19.1% to +1.5% and arm C's from
+10.4% to +1.0%, and both ErC50 biases sit within one per cent throughout. The
middle panels show the three boundary-imposing approaches levelling off: B1 at
+13.1% and B2 at +49.5% on ErC10, and at -8.2% and -11.4% on ErC50. A
...
-10.6% to **-14.8%**, and F's from -5.6% to **-15.6%** — the two largest ErC50
biases in the study at the highest precision tested, larger than B2's -11.4%.
...
precision, falling from +49.3% to +15.7%. Note that
this is not convergence on the truth: it flattens out near +16%, an order of
magnitude away from arm A's +1.5%.
...
## Mechanism of the family-floored approaches
...
F is the one that needs explaining, because its bias changes sign: +14.0% at
...
At the highest precision tested, B3 and E contained the true ErC50 in **none**
of the 500 simulated datasets and B1 in one of them, and B3 and E likewise miss
almost every ErC10 interval (0.000 and 0.126). F is not far behind at 0.116 on
ErC50. The
...
reaches 1.000 — every interval contained the truth — but its RMSE is 0.63
against arm A's 0.38 and its point estimate is 11% low. The intervals are wide
...
B2 carries one further qualification, which the Limitations section sets out and
which is worth flagging where its numbers are read. Every fit in this study
holds `sigma` constant along the curve, and B2 is the approach that suffers most
from that: it leaves genuinely negative observations in place and then forbids
the curve to reach them, so a single residual scale has to cover both them and
the control. Fitted with a dispersion sub-model instead, most of B2's bias, its
interval width and its divergent transitions go away. Its poor showing here is
substantially a property of the homoscedastic fit rather than of the
constraint.
...
fall from 2.18, 1.91 and 1.98 to 1.43, 1.42 and 1.42 as the control coefficient
...
B3 at 1.98 and B2 at 3.04 — corresponding to overestimates of the true `nec` by
...
the sweep, from 3.27 to 2.44. This is not
convergence: it is still 87% high at the finest precision tested, and its ratio
to the intact analysis has *risen* throughout, from 1.43 to 1.69. What is falling
...
* **The NSEC is biased high**, by 34 to 113% relative to the intact analysis at
  the highest precision --- and by 47 to 134% against the true `nec`, which is
  the comparison the previous section makes. Either way the substance appears
  *less* toxic than it is.
...
Two qualifications, both established by the model-averaging sweep reported
below. The direction above is a property of the **single-model** analysis: under
model averaging the NSEC bias reverses to *negative* at high precision, and at
realistic noise it is largely repaired. And the sign is what makes the
single-model NSEC the more serious of the two — under averaging the ErC50 is the
estimate that stays reliable, while the NSEC is the one whose behaviour depends
on how good the measurement is.

### Direction of the B1 contrast

The contrast between B1 and the intact analysis changes sign across the sweep:
B1 sits 4% *below* A in the noisiest scenario and 34% *above* it in the most
precise one. E does the same thing, 6% below and 37% above. Both directions are consequences of the two mechanisms described in
...
contrast with A is close to neutral at high noise (a ratio of 1.01), and a study
that compared only those two at a single realistic noise level could easily have
concluded that nothing was wrong.

<!-- NUMBERS IN THIS SECTION ARE GENERATED, NOT TYPED.
     Source: open-AIMS/negative-sgr at commit 0181d66e --
     analysis/phase10_vignette_numbers.R, whose output is
     analysis/phase10_vignette_numbers.txt, over analysis/phase10_metrics.csv.
     Regenerate there and update this section after any change to the Phase 10
     sweep, and move the pinned commit in "Provenance of the reported results" to
     match. Figures below are at n = 200 per cell (3 cells x 200 x 5 arms,
     27,600 model fits, 0 failures, 0 exclusions).
     Same rule as the rest of the vignette: never edit a result in place. -->

## Model averaging

Everything above fixes one equation in advance. That is the right design for
scoring a convention — the generating model is the one fitted, so the only
misspecification is the one each approach deliberately introduces — but it is
not the workflow `bayesnec` recommends, which is to fit a candidate set and
average over it. The eight-curve figure above suggests why that might matter:
under B3, E and F the curve is held at zero where the data it was given also sit
at zero, and the only way a `nec4param` shape can meet both that floor and the
control is to flatten its descent and push `nec` to the right, which the fitted
`beta` and `nec` show it doing. The toxicity estimates follow. (This is a
different failure from B2's, where the data are *not* floored and the curve
genuinely cannot reach them.) A more flexible equation might follow the same
floored data adequately over the region where ErC10 and ErC50 are actually read,
without ever going below zero.

A separate sweep tested exactly that. Five approaches — A, C, B3, E and F — were
re-run on three of the twelve scenarios, 200 simulated datasets each, with every
approach averaged over the declining candidate set `bayesnec` retains for its
family. The datasets are the same ones the single-model results used, so every
comparison below is paired. Models that failed to converge (R-hat > 1.01) were
dropped and the weights re-stacked over the survivors, which is what the
workflow does; **no approach in any scenario ever lost its whole candidate set**,
so no dataset was excluded from any arm and the comparison is not selective.

One consequence of the pairing is worth stating, because it invites a
cross-check that will otherwise look like an error. Every "before" figure in
this section is the single-model result recomputed over these 200 datasets, not
the 500-dataset figure quoted earlier, so the two differ slightly --- E at a
9.6% control CV is -10.3% here against -10.6% in the precision sweep, and F is
-5.3% against -5.6%. The paired 200 is the right baseline for a paired
comparison; the 500 is the right one for the single-model results on their own.

**The generating model is almost never selected, and it does not matter.** Mean
model weight on `nec4param` — the equation the data were generated from:

| | A | C | B3 | E | F |
|---|---|---|---|---|---|
| control CV 1.9% | 0.62 | 0.36 | **0.001** | 0.007 | 0.002 |
| control CV 9.6% | 0.27 | 0.33 | 0.21 | 0.086 | 0.044 |
| CV 9.6%, `delta` 8 | 0.28 | 0.33 | 0.20 | 0.097 | 0.041 |

The floored approaches load their weight onto `ecxwb2` and `ecxwb1` instead. The
precision dependence is sharp: at a 1.9% control coefficient of variation the
pinned `nec4param` keeps a tenth of one per cent of the weight, because data that
good can see it does not fit.

These are `bnec()`'s default weights — pseudo-BMA with Bayesian bootstrap, set
by `define_loo_controls()` unless `loo_controls` says otherwise — and the default
is what makes the sentence above readable. Stacking, `loo_model_weights()`'s own
default, solves for the best-predicting mixture and will zero a model because it
is redundant given the others just as readily as because it is wrong, so a small
stacking weight says little about a single model. A pseudo-BMA weight is
proportional to expected log predictive density, so a model reduced to a
thousandth of the weight has been judged to fit much worse, not merely to have
been covered by something else.

**ErC50 is rescued in every scenario tested.**

| ErC50 relative bias | CV 1.9% | CV 9.6% | CV 9.6%, `delta` 8 |
|---|---|---|---|
| B3 | -10.2% → **-3.0%** | -7.6% → **-3.3%** | -7.8% → **-3.3%** |
| E | -14.8% → **-5.5%** | -10.3% → **-5.0%** | -9.8% → **-4.9%** |
| F | -15.7% → **+0.9%** | -5.3% → **+1.5%** | -13.3% → **+2.4%** |

Coverage recovers with it — B3 from 0.00 to 0.58, 0.58 to 0.80 and 0.48 to
0.80 — and, importantly, **RMSE falls at the same time**, from 0.52 to 0.17 for
B3 at the highest precision. Intervals widen by only 1.1 to 1.6 times, and for F
at high precision they *narrow*. This is accuracy gained rather than coverage
bought with width, which is the distinction the B2 result above exists to warn
about.

**The NSEC divides on measurement precision, and this is the more interesting
half.** At the noise levels of a real test, averaging repairs it: B3's bias moves
from +72.9% to **-1.7%** and its coverage from 0.07 to 0.98 at a 9.6% control CV,
and from +41.4% to **-6.0%** with coverage 0.10 to 0.98 at `delta` = 8. Every
approach improves on bias, coverage and RMSE together.

At the highest precision tested it does the opposite. B3 moves from +52.7% to
**-62.7%**, its coverage from 0.00 to **0.01** — two of two hundred intervals
contained the truth — and its **RMSE worsens**, 0.69 to 0.82. It is the one place
in this study where averaging leaves an approach worse on every measure at once.
E (-42.1%) and F (-53.5%) do the same thing less severely.

That split is the same distinction the precision sweep draws throughout this
vignette, now applied to the workflow rather than to the convention. **A
discrepancy that shrinks as the noise shrinks is estimation error; one that does
not is misspecification.** Averaging removes the estimation component of the
NSEC bias, which is what dominates at realistic noise, and in doing so exposes a
misspecification component that only dominates once the measurement is good. The
single-model fits carried both at once and the noise axis alone could not
separate them.

**ErC10 is where averaging can cost you.** F moves from +15.7% to **-23.7%** at
the highest precision, its coverage falling from 0.98 to 0.50, and from +10.1% to
-10.6% at `delta` = 8. Even the intact analysis is not immune: arm A's ErC10 goes
from +19.0% to -11.3% at a 9.6% control CV, with RMSE rising from 0.48 to 0.52.
ErC10 asks a 10% decline to be resolved against the residual scatter, and
averaging over shapes that disagree about the shoulder is not obviously the right
way to do that. Do not summarise these approaches across endpoints.

### Interpretation

The practical reading is narrower than "averaging fixes it", and narrower than
"flooring is fatal".

* **Do not floor.** Nothing here rehabilitates the practice. The intact and
  censored analyses are unbiased on ErC50 under both workflows, and they need no
  rescue.
* **If you have floored data and cannot recover it**, averaging over a candidate
  set is markedly better than fitting one equation, on ErC50 in every scenario
  tested and on the NSEC at realistic noise. Do not fix a single model on floored
  data.
* **The failure does not go away; it relocates.** The clean result is that the
  flooring penalty is worst where the measurement is best, and that survives
  averaging. A laboratory that improves its technique still ends up with an
  analysis that is more confident and no less wrong — and at high precision
  averaging makes the NSEC *worse* rather than better.
* **A good model weight is not a good model.** The floored approaches recover
  ErC50 while giving the true equation a tenth of one per cent of the weight.
  Weights describe which shape fits the data in front of them, not which process
  generated it, and a floored dataset is not the process.
...
approaches. They ship with the package as `alga`, so everything in this section
is computed here rather than transcribed: two tests on *Cladocopium proliferum*,
a coral endosymbiont, over seven days, and two on *Rhodomonas salina* over three
days, each species exposed to two contaminants on undisclosed and mutually
incomparable concentration scales. E and F are simulation-only: on a real dataset
there is no true value to score them against, and what they add to the study —
that the bias survives to the noise-free limit — can only be shown where the
truth is known.
...
# rate that substitution should have carried, and it is what the intact analysis
# uses in place of the substituted zero.
alga$bound <- (log(10) - log(alga$density_initial)) / alga$days
alga$below_limit <- alga$sgr_source == "substituted"

datasets <- alga |>
  group_by(dataset, species, duration_d = days) |>
  summarise(n = n(), n_conc = n_distinct(dose),
            control_rate = mean(sgr[dose == 0]),
            below_limit = sum(below_limit),
            prop_at_or_below_zero = mean(sgr <= 0), .groups = "drop") |>
  mutate(fold_change_R = exp(control_rate * duration_d), .before = below_limit)
```



Table: The four case-study datasets, computed from `alga`. All are in the regime where the design reaches the zero-growth boundary. `Below limit` counts the rows whose population fell below the counting limit, for which the source substituted a zero.

|Dataset       |Species      | Days|  n| Concentrations| Control rate (per d)| Fold change R| Below limit| Prop. at or below zero|
|:-------------|:------------|----:|--:|--------------:|--------------------:|-------------:|-----------:|----------------------:|
|c_proliferum  |c_proliferum |    7| 85|             14|                0.119|          2.31|           0|                  0.141|
|c_proliferum2 |c_proliferum |    7| 70|             13|                0.169|          3.27|           0|                  0.143|
|r_salina      |r_salina     |    3| 85|             14|                1.431|         73.12|          16|                  0.235|
|r_salina2     |r_salina     |    3| 70|             13|                0.951|         17.32|           4|                  0.300|
...
rate is *undefined* rather than negative. Those rows carry
`sgr_source == "substituted"` in `alga`, 16 on `r_salina` and 4 on
`r_salina2`; the two *Cladocopium* tests have none, and their negative values are
measurements. There is no measured value to retain,
...
That also settles what arm C means on these two datasets. Left-censoring
...
and came out negative, since withdrawing its magnitude is the convention under
test, but the counting limit itself for a row that fell below it, which is a
stronger statement than "below zero" and is the one thing the test does
establish about that row. Censoring such a row at zero instead moves the ErC50
by 3 to 6% and the NSEC by 26 to 30%, in both cases away from the value the
retained-measurement arms give.

Unlike the simulation, these fits are run by this vignette. They are the same
six Gaussian approaches, built the same way — one shared prior taken from the
intact analysis, so that a change of arm is a change of likelihood and nothing
else — applied to each dataset in turn.


``` r
# One dataset's six arms. The prior is pulled from A and passed to the other
# five for the reason given in the Methods: bnec() derives the default Gaussian
# priors from the response vector, and on r_salina2 the substituted zeros pull
# the default bot prior onto zero, so letting each arm take its own default
# would leak the convention under test into the prior.
fit_arms <- function(d) {
  # The intact analysis restores the counting-limit rate where the source
  # substituted a zero. On the two Cladocopium tests nothing is substituted and
  # this is the measured series unchanged.
  d$y <- ifelse(d$below_limit, d$bound, d$sgr)

  fit_A <- bnec(y ~ crf(dose, "nec4param"), data = d,
                family = gaussian(link = "identity"), seed = 333,
                open_progress = FALSE)
  shared <- get_priors(fit_A)
  pinned <- shared
  pinned$prior[pinned$nlpar == "bot"] <- "constant(0)"

  d_floored <- transform(d, y = pmax(y, 0))
  # Left-censoring declares only that the truth lies at or below a bound, so the
  # bound should be the tightest one the measurement supports. For a measured
  # negative that is zero -- withdrawing the magnitude is the convention under
  # test. For a row whose population fell below the counting limit it is the
  # limit itself, which is a stronger statement than "below zero" and is what is
  # actually known; censoring such a row at zero would discard the one thing the
  # test does tell you about it. See the Limits of the censored likelihood
  # section of the Single model usage vignette.
  d_cens <- transform(d,
    cens = ifelse(below_limit | y < 0, "left", "none"),
    y = ifelse(below_limit, bound, pmax(y, 0)))
  # D's cut-off is the lowest tested concentration at which A's own posterior
  # mean has reached zero, which is how the approach is defined -- it cannot be
  # applied without first fitting A.
  mu_A <- fitted(fit_A, newdata = data.frame(dose = d$dose))[, "Estimate"]
  cross <- suppressWarnings(min(d$dose[mu_A <= 0]))

  list(
    A  = fit_A,
    C  = bnec(y | cens(cens) ~ crf(dose, "nec4param"), data = d_cens,
              family = gaussian(link = "identity"), prior = shared,
              seed = 333, open_progress = FALSE),
    D  = bnec(y ~ crf(dose, "nec4param"), data = subset(d, dose < cross),
              family = gaussian(link = "identity"), prior = shared,
              seed = 333, open_progress = FALSE),
    B1 = bnec(y ~ crf(dose, "nec4param"), data = d_floored,
              family = gaussian(link = "identity"), prior = shared,
              seed = 333, open_progress = FALSE),
    B2 = bnec(y ~ crf(dose, "nec4param"), data = d,
              family = gaussian(link = "identity"), prior = pinned,
              seed = 333, open_progress = FALSE),
    B3 = bnec(y ~ crf(dose, "nec4param"), data = d_floored,
              family = gaussian(link = "identity"), prior = pinned,
              seed = 333, open_progress = FALSE))
}

real_fits <- lapply(split(alga, alga$dataset), fit_arms)
```


``` r
# ErC50 and NSEC from every fit, with the tested concentration range carried
# alongside so that an interval running to the edge of the design can be
# flagged rather than read as an estimate.
real_estimates <- do.call(rbind, lapply(names(real_fits), function(ds) {
  rng <- range(alga$dose[alga$dataset == ds & alga$dose > 0])
  do.call(rbind, lapply(names(real_fits[[ds]]), function(a) {
    f <- real_fits[[ds]][[a]]
    e <- ecx(f, ecx_val = 50, type = "absolute")
    n <- nsec(f)
    rbind(data.frame(dataset = ds, arm = a, estimate = "ErC50",
                     val = e[1], lo = e[2], hi = e[3]),
          data.frame(dataset = ds, arm = a, estimate = "NSEC",
                     val = n[1], lo = n[2], hi = n[3])) |>
      transform(x_min = rng[1], x_max = rng[2])
  }))
}))

# Expressed as a ratio to the intact analysis on the same dataset, because the
# four tests are on undisclosed and mutually incomparable concentration scales.
# An estimate is flagged unusable where its interval has collapsed to a point --
# truncation can leave too few concentrations above the crossing to support one
# -- or has run to an edge of the tested range, where the bound is the design
# rather than the data.
real_ratios <- real_estimates |>
  group_by(dataset, estimate) |>
  mutate(ref = val[arm == "A"],
         r = val / ref, rlo = lo / ref, rhi = hi / ref,
         usable = !(lo <= x_min | hi >= x_max | hi - lo < .Machine$double.eps^0.5)) |>
  ungroup() |>
  select(dataset, arm, estimate, r, rlo, rhi, usable)
```



Because the four datasets have toxicity values on completely different
concentration scales, each estimate is shown as the ratio computed above — the
approach's estimate divided by the intact analysis on the same dataset. A ratio
of 1 means the approach agreed with the intact analysis; a ratio of 2 means it
returned twice the toxicity value.


``` r
real_ratios$arm <- factor(real_ratios$arm, levels = rev(arm_levels))

# B2's lower bound runs to the bottom of the tested range on two datasets, which
# on a log axis is four decades and compresses every other estimate onto the
# reference line. Those estimates are already flagged unusable, so the bar is
# clipped to the panel and the open symbol carries the meaning. Clipping by
# pmax/pmin on the data rather than a scale limit, to avoid depending on
# `scales` for an oob handler.
ratio_lim <- c(0.1, 4)
real_ratios$rlo_p <- pmax(real_ratios$rlo, ratio_lim[1])
real_ratios$rhi_p <- pmin(real_ratios$rhi, ratio_lim[2])
...
<div class="figure" style="text-align: center">
<img src="vignette-fig-real-ratios-1.png" alt="Estimated ErC50 and NSEC from each approach on the four real datasets, expressed as a ratio to the intact analysis (A) on the same dataset, with 95% credible intervals. The vertical line at 1 is exact agreement with the intact analysis. Open symbols mark estimates flagged as unusable: for B2 the interval reached the edge of the tested concentration range, and for D on two datasets truncation left too few concentrations above the crossing to estimate an interval at all, so the interval is degenerate. Intervals extending past the plotted range are clipped at the panel edge." width="95%" />
...
Three features of that figure require care in
what they do and do not establish.

**The choice changes the answer, substantially.** On `c_proliferum` the floored
analyses return an ErC50 1.8 to
1.9 times that of the intact
analysis, and the pinned analysis returns 0.75 times it — a
spread of about 2.5-fold
between approaches applied to the same data. On `r_salina` the pinned analysis returns
0.43 times the intact ErC50 and 0.25
times its NSEC. These are not rounding differences; they are the difference between one
regulatory value and another.
...
on `c_proliferum2` they are within
6% of it. The simulation is unambiguous that
...
estimates are shown as open symbols. Its intervals on the remaining two are
2.7 and
116 times wider than the
intact analysis. Pinning the asymptote at zero while
...
NSEC between 1.8
and 2.5 times
the intact one — the same upward direction, and a
...
flooring loses — an ErC50 bias of -0.3% against the intact analysis's +0.6% —
...
and -15.6%, against -11.4% for the worst of the deliberate conventions. Neither
gives any sign of trouble: across all 6,000 of its fits, E produced not one
...
bias taken across scenarios: F averages -1.6% on ErC50 in the reaching regime by
combining +14.0% at `delta` = 2 with -13.4% at `delta` = 8, and its RMSE, 1.11
against arm A's 0.38, is what gives that away.
...
reaching regime the intact analysis holds 0.89–0.95 and left-censoring 0.77–0.85
...
is a real misspecification and it is shared by all eight approaches. It is not
forced: `bnec()` carries a dispersion sub-model, and for a response that crosses
zero the available form is `disp("loglinear")`, which is linear in the mean
rather than in `log(mu)` --- `disp("power")` is refused for exactly this case.
See the *Non-constant dispersion* section of the [Single model usage][e1]
vignette. The sweep predates that interface and was run without it, and the
coverage figures above should be read as reflecting a single pooled `sigma`
standing in for a residual scale that varies 8.1-fold along the curve.

This has been checked rather than left as a worry, on scenario 8 --- 50
datasets, the six Gaussian approaches fitted twice each, once homoscedastic and
once under `disp("loglinear")`, paired so every contrast is within-dataset. The
result splits the approaches in two, and it does not fall where the rest of this
vignette would lead you to expect.

**The case against flooring is unaffected.** B1's ErC50 bias moves from -9.0% to
-8.5% and B3's from -10.7% to -9.8%, with coverage still 0.50 and 0.44. Flooring
damages the mean structure, and modelling the residual scale does not repair
that.

**B2 is largely exonerated.** Its ErC50 bias moves from -12.7% to **-1.2%**, its
NSEC overestimate from 1.49 times the intact analysis to **1.06**, and its
divergent transitions from 3.22 per fit to 0.12. All three of the symptoms
reported for B2 above have one cause, and it is not the pinned asymptote: it is a
single `sigma` being asked to cover both the control scatter and observations the
constrained curve cannot reach. Given a dispersion sub-model, pinning `bot` on
data that were never floored is close to unbiased. **Read B2's results above as a
property of the homoscedastic fit rather than of the constraint.**

**The direction of the B1 contrast is also a property of the homoscedastic
fit.** The account given above --- flooring compresses `sigma`, raises the
control reference quantile and pulls the NSEC crossing left --- depends on
`sigma` being *global*. Under a dispersion sub-model the control-region scale is
estimated where the control data are, flooring has removed nothing there, and
the channel closes: B1 moves from 1.01 to 1.21 and B3 from 1.05 to 1.28, from
level with the intact analysis to clearly above it.

One further observation, unanticipated and not chased down: floored data and a
dispersion sub-model sample badly together. B3 draws 13.8 divergent transitions
per fit under `disp("loglinear")` and B1 4.1, against none at all when
homoscedastic --- so which approach the sampler complains about reverses along
with everything else.

This is one scenario at one noise level, and `"loglinear"` is itself an
approximation to the linear `sigma` ramp the data were generated from. It shows
the ordering is dispersion-dependent for B2; it does not re-score the study.
Re-running the full precision sweep under `disp("loglinear")` would, and is not
done here. The check is recorded in `notes/disp_sensitivity_example7.md`, with
its script and full output alongside.
