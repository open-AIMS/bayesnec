...
Specific growth rate can legitimately be negative: a population that declines
over a test has a negative one. The common conventions for handling those
negative values before fitting are applied routinely and are rarely reported.
Six are compared here, each named below by the short label used for it
throughout: keeping the values as recorded (`measured`), deleting the rows that
hold them (`deleted`), replacing the values with zero (`floored`), declaring
them left-censored at zero (`censored`), and fitting a response distribution
with no support below zero, either a Beta (`beta`) or a Gamma (`gamma`).

Three toxicity estimates are reported for each. The ErC10 and the ErC50 are the
concentrations at which the growth rate has fallen to 90% and 50% of the
control, and the NSEC is the highest concentration at which the modelled
response is still indistinguishable from the control. Sections 2.2 and 2.3
define all three.

## The problem

Replacing a negative growth rate with zero asserts that the population neither
grew nor declined. That is a statement about the measurement, not a
presentational tidy-up. It is made on exactly the observations that determine
the shape of the curve at the top of the concentration series. Nothing in a
Gaussian concentration-response fit requires the response to be positive, so the
substitution is not something the fitting method demands.

## Findings

Six conventions were scored against a known truth on 4,200 model-averaged fits,
across a precision sweep in which the control coefficient of variation falls
from 9.6% to 1.9%. Each convention calls `bnec()` with its own default priors
and averages over the declining candidate set, so the comparison is between
practices rather than between likelihoods.

- Keeping the measurement recovers the truth and replacing it does not. The
  ErC50 bias of `measured` runs from -0.4% to
  -0.2% across the sweep, and of `censored`
  from -0.7% to
  -0.2%. Flooring stops at
  -2.3% and a Beta at
  -5.8%.
- The error does not shrink as the experiment improves. A bias that does not
  converge sits inside an interval that keeps narrowing, so coverage collapses:
  at the highest precision tested the 95% interval of `floored` contains the
  true ErC50 in 0.24 of
  realisations, against 0.97 for
  `measured`. A laboratory that adds replicates and reduces its control
  variability produces an analysis that is more confident and no less wrong.
- The no-effect estimates are affected most. The NSEC of `floored` is
  displaced by -52.9% at the highest precision
  and its interval contains the true `nec` in
  0.00 of 100 realisations.
- A zero-bounded distribution is not a solution. The Gamma displaces ErC10
  by -25.0% with coverage of
  0.01, because an identity-link
  Gamma ties the residual spread to the fitted mean and these data were
  generated with a constant one.
- None of it is visible in the diagnostics. Every one of the 4,200 fits
  converged and returned all three estimates.

## Recommendations

- Analyse the growth rates as measured, and let the model set decide the shape.
- Where the negative values were never recorded, declare them left-censored at
  zero rather than substituting a value.
- Do not replace negative values with zero, and do not choose a zero-bounded
  response distribution in order to avoid them.
- Do not convert to percent inhibition before fitting.
- Report the proportion of observations at or below zero, which is the single
  number that tells a reader whether the question arises for the dataset at all.

Section 7 gives these with the evidence for each, and Section 8 the limitations.
...
The removal is done in one of several ways: by deleting the rows that hold
them, by replacing the negative values with zero before fitting, by declaring
them censored at zero, or by fitting a response distribution such as a Beta or
a Gamma that has no support below zero. In a minority of cases the negative
values are kept and the model is fitted to the data as recorded, which is the
`measured` convention of Section 5.

Here we examine how each of those conventions affects the toxicity estimate
that is subsequently reported.

- Section 2 sets out how the estimates are defined, and why the definition
  matters when the response can cross zero.
- Section 3 sets out what replacing a negative value commits the analyst to,
  which is more than an edit to the data.
- Section 4 records what `bayesnec` permits, which changed in version 2.2.0.
- Section 5 reports a simulation study in which the true values are known, so
  that each convention can be scored rather than merely compared.
- Section 6 applies the same comparison to four real algal growth tests.
- Sections 7 and 8 give the recommendations and the limitations.
- Section 9 records the provenance of every figure quoted.
...
The `r` in ErC50 names the endpoint the estimate was derived from. It comes
from OECD Test Guideline 201 [@oecd2026tg201], which writes it as a subscript;
this vignette writes it inline. In OECD terminology the *endpoint* is the
measured biological response, and TG 201 defines two of them from the same cell
counts and requires both to be reported. ErC50 is the EC50 for the average
specific *growth rate*, the rate constant of population increase, and EyC50 is
the EC50 for the *yield*, the biomass increase over the test. For the same data
EyC50 is systematically lower than ErC50, and the guideline requires the growth
rate to be reported because it does not depend on test duration.

An ECx or an NSEC, both defined below, is therefore a *toxicity estimate*,
not an endpoint. That
...
highest concentration at which the modelled mean response is still statistically
indistinguishable from the modelled control response. Read up the concentration
series, it is the point at which the response first becomes distinguishable from
the control. `bayesnec` takes a lower quantile of the control posterior — by
default the 1st percentile, controlled by `sig_val` — and finds where each
posterior draw of the curve crosses it. It
...
the results below have to be read. It follows from how the metric is defined,
not from how any approach fits the data. A noisier experiment produces a wider
control posterior, hence a lower reference value, hence a crossing further to
the right, at a higher concentration. As the residual error among the control
replicates tends to zero the reference converges on the control mean, and the
crossing is found at a lower concentration.

Several equations in `bayesnec` include a `nec` parameter: a threshold
concentration below which the modelled mean response is flat at the control
level and above which it declines, so the curve is two straight or curved
segments meeting at a corner rather than a single smooth decline. For such an
equation the NSEC converges on `nec` itself as the residual error falls. The
true `nec` is therefore the NSEC's limiting value, not its expected value at any
finite level of noise.
...
No available distribution family represents that assertion exactly. A floored response has a
...
now a matter of evidence rather than of specification. Whether to replace the
measurements is a separate decision, taken before any equation is fitted, and it
is the one the sections that follow are about.
...
and the default model averaging over that set — pseudo-BMA with a Bayesian
bootstrap, which is what `bnec()` uses when `loo_controls` is not supplied. No
convention is handed a prior or an equation chosen by another. The contrast between two conventions is therefore the
...
but the replication per concentration can change, and at the top of the series,
at the highest concentrations, a whole treatment group can disappear.
...
## 5.3 One dataset under each convention




A summary over 100 realisations of a cell, which is what the tables and figures
below report, does not show what a convention does to a dataset. All six
conventions were therefore also fitted to a single simulated dataset from `p1`,
the noisiest cell tested, in which
10 of the seventy
design points have a true mean growth rate at or below zero and
11 of the seventy simulated observations came out
negative. The realisation is the one drawn for the `p1` exemplar by
a seed fixed on the cell and convention names, so it was chosen before any
result was seen rather than selected afterwards for the picture it makes.



``` r
ggplot(panel_fit, aes(x = x)) +
  geom_hline(yintercept = 0, linetype = "dashed", colour = "grey60") +
  geom_vline(xintercept = panel_cell$ErC50, linetype = "dashed",
             colour = "grey60") +
  geom_ribbon(aes(ymin = lo, ymax = hi, fill = arm), alpha = 0.25) +
  geom_point(data = subset(panel_points, x > 0),
             aes(y = y, shape = altered), size = 1.1, alpha = 0.7) +
  geom_line(data = panel_true, aes(y = y), linewidth = 1, colour = "grey10") +
  geom_line(aes(y = y, colour = arm), linewidth = 0.8) +
...
<div class="figure" style="text-align: center">
<img src="vignette-fig-one-dataset-1.png" alt="**Figure 2.** One simulated dataset from cell `p1`, fitted under each of the six conventions. Points are the data each convention was given; open symbols mark the 11 observations it altered or dropped, drawn at zero by the four conventions that substitute and at their measured values under `deleted`, which discards those rows. `measured` alters none, so nothing in its panel is marked. The coloured line is the model-averaged posterior median and the band its 95% credible interval; the black line is the true curve. The dashed horizontal line is zero growth and the dashed vertical line the true ErC50. The controls sit at a concentration of zero and have no position on a log axis, so they are not drawn; every fit uses them." width="100%" />
<p class="caption">**Figure 2.** One simulated dataset from cell `p1`, fitted under each of the six conventions. Points are the data each convention was given; open symbols mark the 11 observations it altered or dropped, drawn at zero by the four conventions that substitute and at their measured values under `deleted`, which discards those rows. `measured` alters none, so nothing in its panel is marked. The coloured line is the model-averaged posterior median and the band its 95% credible interval; the black line is the true curve. The dashed horizontal line is zero growth and the dashed vertical line the true ErC50. The controls sit at a concentration of zero and have no position on a log axis, so they are not drawn; every fit uses them.</p>
</div>


Under `measured` the model average follows the true curve the whole way down,
including the part below zero, and its credible band is narrow throughout. Under
`censored` the median does the same; the band flares once the curve passes zero,
because a row declared to lie below a bound says less about where the curve goes
than a measurement does.

The three conventions that put a zero in place of the measurement — `floored`,
`beta` and `gamma` — produce a curve that flattens onto zero instead of
continuing down. The flattening is visible from a little below the true ErC50,
which is marked by the dashed vertical line, so it begins at lower
concentrations than the substituted rows themselves. That is the mechanism
behind the biases reported below: the substitution is made at the top of the
series, and the curve that results is displaced over a range that includes the
concentrations the estimates are read from. The displacement is small at the
ErC50 and large at the bottom of the curve, which is where the NSEC is read, and
that ordering is what Sections 5.4 and 5.6 measure.

The `deleted` panel shows a different consequence. Removing every row with a
negative value removed the highest concentrations entirely, so the fitted curve
stops where the surviving data stop and the design is no longer the one that was
run.

The `gamma` panel shows a failure of the variance rather than of the mean. Its
credible band is widest at the control, on the left, where the generated
residual spread is no wider than anywhere else, because an identity-link Gamma
ties the spread to the fitted mean. Section 5.4 quantifies that with a posterior
predictive check.
...
<div class="figure" style="text-align: center">
<img src="vignette-fig-precision-bias-1.png" alt="**Figure 3.** Relative bias in the estimated ErC10 and ErC50 against the control coefficient of variation, which decreases to the right as the experiment becomes more precise. Each point summarises 100 simulated datasets. The dashed line is zero bias." width="100%" />
<p class="caption">**Figure 3.** Relative bias in the estimated ErC10 and ErC50 against the control coefficient of variation, which decreases to the right as the experiment becomes more precise. Each point summarises 100 simulated datasets. The dashed line is zero bias.</p>
</div>
...
-2.4% to
0.0%. All three are within half a per cent of
...
The reason is in the variance rather than in the mean, and a posterior
predictive check locates it. These data are generated with a residual standard
deviation that is constant along the curve, so what a correctly specified family
should predict is known exactly: the generated value at every concentration, and
therefore a ratio of one between the predicted spread at the control and at the
highest concentration. Each fit of the four precision cells was refitted and its
model-averaged predicted spread compared with the generated one, 100
realisations per cell and convention.



`measured` is correctly specified by construction, so what it returns is the
closest this statistic comes to one on these data and the reference every other
convention is read against: a control ratio of
1.00 to 0.98, with 95% of
realisations between 0.81
and 1.18, and a
gradient of 1.03.

An identity-link Gamma cannot reproduce a constant spread, because it ties the
residual standard deviation to the mean. It over-states the spread at the
control by a factor of 2.51 to 2.69,
and its gradient runs from 181
at the noisiest cell to 90
at the most precise, against a true value of one. The floored block is therefore
given almost all of the weight in the likelihood and the control scatter almost
none, and the shoulder — which is where a 10% decline is read — is what gives
way.

That also locates what flooring does and does not damage. Under a Gaussian
family the floored data are described by a correctly specified variance model:
`floored` returns a control ratio of 0.91 to
1.12 and a gradient of
1.04, both inside the reference. What
the substitution changes is the mean structure, which is what Section 5.7
reports. Under a Gamma the two failures compound.

The Beta is a third case. Its control ratio is 1.09 to
1.10, inside the reference, and its gradient
1.71 to 2.66
— a mild departure rather than the Gamma's. Whatever displaces its ErC10 is
therefore not the variance model.

The Gamma's intervals are
2.14
...
## 5.5 Credible interval coverage
...
<div class="figure" style="text-align: center">
<img src="vignette-fig-precision-coverage-1.png" alt="**Figure 5.** Proportion of 95% credible intervals containing the true ErC10 or ErC50, against the control coefficient of variation. The dashed line is nominal coverage." width="100%" />
<p class="caption">**Figure 5.** Proportion of 95% credible intervals containing the true ErC10 or ErC50, against the control coefficient of variation. The dashed line is nominal coverage.</p>
</div>
...
is 3.2%, of `censored`
2.4% and of `deleted`
0.7%. They are not flat along the way — the
...
of `floored` runs from -11.9% to
-52.9%, of `beta` from
-1.4% to -42.7%, and
of `gamma` from 25.0% to
-71.6%. Coverage follows: at the highest precision
...
from 0.99 to
0.88. That is the property of the
...
3.2% has closed, so an interval that is
...
## 5.7 The equations the data are made to resemble
...
Table: **Table 2.** Mean model weight on the generating equation `nec4param` across the precision sweep, averaged over 100 realisations per cell. The remaining weight is spread over the other eleven to thirteen equations in the candidate set.
...
<div class="figure" style="text-align: center">
<img src="vignette-fig-real-ratios-1.png" alt="**Figure 7.** Estimated ErC10, ErC50 and NSEC from each convention on the four real datasets, expressed as a ratio to the `measured` analysis of the same dataset, with 95% credible intervals. The vertical line at 1 is exact agreement. Open symbols mark estimates flagged as unusable, meaning the interval reached an edge of the tested concentration range, where the bound is the design rather than the data, or collapsed to a point. Intervals extending past the plotted range are clipped at the panel edge." width="100%" />
<p class="caption">**Figure 7.** Estimated ErC10, ErC50 and NSEC from each convention on the four real datasets, expressed as a ratio to the `measured` analysis of the same dataset, with 95% credible intervals. The vertical line at 1 is exact agreement. Open symbols mark estimates flagged as unusable, meaning the interval reached an edge of the tested concentration range, where the bound is the design rather than the data, or collapsed to a point. Intervals extending past the plotted range are clipped at the panel edge.</p>
</div>

No estimate in Figure 7 is flagged unusable: every interval lies inside the tested concentration range.

Three features of Figure 7 require care in what they do and do not establish.
...
the weaker evidence. The simulation does not have this problem. Averaged over its
4,200 fits, equations failing either diagnostic hold 0.4% of the model weight,
and the largest figure for any one cell and convention is 5.2%, under `gamma` in
the `ctl` cell, so no figure reported from the simulation depends on an equation
that did not converge.

The weight on the zero-asymptote equations is the diagnostic Section 5.7
...
-52.9%. Averaging over the enlarged candidate
...
was present in the earlier version of this study, but it means no convention
reported here estimates a dispersion that varies along the curve. One of the
four `alga` datasets does show a residual gradient, so the homoscedastic case is
not the only one that arises.

The precision sweep was also run with a dispersion sub-model, on the same cells
and realisations, and those results are not reported. A sub-model does not
repair the conventions that impose the zero boundary through the response
distribution, and adding one to a Gaussian, whose variance model is already
correct, gives it a gradient the data do not have. The compendium holds the code
and the tables, and its `CLAUDE.md` records why they are uncited.
...
reported bias is a median of
0.4% of the true value for
ErC50, 1.2% for ErC10 and
2.2% for the NSEC, reaching
10.5% for the
gamma NSEC in the
d2 cell. Differences smaller than those are
not resolved by this study. The Monte Carlo standard error on a coverage of 0.95
is 0.02.
...
measures is what package defaults do. Three of the changes between that commit
and the 2.1.3.1 release are the subject of results reported above. The `nec` and
`ec50` priors are now built on the log of the predictor, and the initial-value
search accepts chains individually. The NSEC crossing is now sought from the
control upward, with the control made the first point of the searched grid, so a
draw that crosses between the control and the next grid point is no longer lost;
this lowers the reported lower bound of an NSEC interval by about three times as
much as it changes the point estimate, and leaves every ECx unchanged. An
analysis run under the CRAN release will not reproduce these numbers exactly.

The posterior predictive figures quoted in Section 5.4 are transcribed from
`results/ppc_spread.csv`, which summarises 100 realisations of each cell and
convention. They cover the four precision cells only. The study also keeps one
fitted object per cell and convention, and all 24 case fits, chosen by a seed
fixed on the cell and convention names before any result was seen; those are not
in the compendium's repository — 66 model-averaged fits is too much to track —
and are fetched from the cluster with `hpc/fetch-fits.sh`.
