...
The reason is in the variance rather than in the mean, and it is visible in a
posterior predictive check. These data are generated with a residual standard
deviation that is constant along the curve, so what each family should predict
is known exactly. On the saved fit for the noisiest cell, the two Gaussian
conventions do predict it: `measured` and `floored` both return a predicted
spread of 0.86 to 0.93 times the generated 0.0114 at every concentration, and
the ratio of their predicted spread at the top of the curve to that at the
bottom is 1.02 and 0.98 against a true value of 1. An identity-link Gamma
cannot, because it ties the residual standard deviation to the mean: it
over-states the spread at the control by a factor of 3.5, understates it at the
shoulder, and collapses it to zero at the floored values, a top-to-bottom ratio
of five thousand. The floored block therefore carries almost all of the weight
in the likelihood and the control scatter almost none, and the shoulder — which
is where a 10% decline is read — is what gives way.
...
what the substitution changes is the mean structure, which is what Section 5.6
...
The posterior predictive figures quoted in Section 5.3 come from the saved fits.
The study keeps one fitted object per cell and convention, and all 24 case fits,
chosen by a seed fixed on the cell and convention names before any result was
seen. They are not in the compendium's repository — 66 model-averaged fits is
too much to track — and are fetched from the cluster with `hpc/fetch-fits.sh`.
