...
That is a decision rather than a limitation, and it has been tested: a separate
sweep re-ran a subset of these scenarios with every approach averaged over a
candidate set instead of fixed. It changes the answer, and the section
*Does model averaging rescue the floored approaches?* below reports what it
...
Two consequences are worth stating plainly, because a reader cannot see them
from the code shown here. The numbers are **frozen**: they describe `bayesnec`
...
precision, falling from +49.3% to +15.7%, and it is worth being explicit that
...
the sweep, from 3.27 to 2.44, and it is worth saying plainly that this is not
...
<!-- NUMBERS IN THIS SECTION ARE GENERATED, NOT TYPED.
     ...
     sweep, and move the pinned commit in "Where these numbers come from" to
...
## Does model averaging rescue the floored approaches?
