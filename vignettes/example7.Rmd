...
the `ecx()` call path common to all six approaches, so that nothing but the
constraint changes between them.
...
**The model set actually fitted.** Every fit reported in this vignette — all
six approaches, all twelve simulation scenarios and all four case studies — used
the single model `nec4param` with `family = gaussian(link = "identity")`. No
model averaging was performed and no model was selected from a set. This is
...
six approaches, giving 12 x 240 x 6 = 17,280 model fits. The iteration count was
...
in the set. It is B2 — which leaves the data untouched and constrains the curve —
that struggles, by two orders of magnitude. The reason is that B2 is asked to fit
a curve that cannot reach its own observations, so the sampler must trade the
residual scale against the curve shape with no good compromise available. A
floored dataset is certainly no longer Gaussian, but under B1 and B3 the model
accommodates it comfortably and reports nothing amiss, which is precisely what
makes those two approaches dangerous: the failure is silent.
