...
The consequence is not the same for the two toxicity estimates. **The ErC50
ordering does not depend on it**: flooring deforms the mean structure, and it
does so whether or not the residual scale is modelled. **The NSEC mechanism
does.** The account given above of why B1 sits below the intact analysis at high
noise runs through the residual scale --- flooring removes the most extreme low
values, compresses `sigma`, raises the control reference quantile and pulls the
crossing left. That channel exists only because `sigma` is *global*. Under a
dispersion sub-model the control-region scale is estimated locally, where
flooring has removed nothing, so the channel should close and the sign of the
B1 contrast at high noise may not survive. The same argument applies to B2,
whose inflated `sigma` is a global response to observations the curve cannot
reach. Re-running the precision sweep under `disp("loglinear")` is the way to
settle it and is not done here.
