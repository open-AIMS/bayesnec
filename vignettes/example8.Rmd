...
Admitting a difference in shape is not a reason to fit one. What separates these
plates is a vertical scale factor, and `ogl()` states that and nothing more: one
deviation on the fitted value, every curve parameter shared. `pgl()` would place
a deviation on each parameter instead, so that the threshold and the slope could
differ from plate to plate as well as the level, and it would estimate four
group-level standard deviations where `ogl()` estimates one. Seventeen plates
would support that --- five levels is the conventional minimum --- so the choice
is about what the design says rather than what it can afford. The traces below
give no reason to fit a shape difference, and the same figure is what would show
...
<div class="figure" style="text-align: center">
<img src="vignette-fig-fig-pam-thresholds-1.png" alt="plot of chunk fig-pam-thresholds" width="85%" />
<p class="caption">plot of chunk fig-pam-thresholds</p>
</div>
