...
The divergences separate the two sets and R-hat does not. Neither
three-parameter equation records a divergent transition; `nec4param` records 71
and `ecx4param` 243. Every one of the four has an R-hat between 1.004 and 1.018,
so a check on R-hat alone would have passed all of them.

The figure shows where the four-parameter fits are unconstrained. Their credible
band spans most of the response range between about 3 and 11 µg/L, which is the
gap between the fourth diuron concentration and the fifth, and closes again where
the observations resume. The extra parameters are asked to describe a region the
design does not sample.
