...
it holds near the control before falling. EC10 changes with it, from 0.278 mg/L
(0.206--0.395) to 0.771 (0.015--1.815) --- almost three times the estimate, on
an interval nine times wider.
...
either way, 0.328 mg/L pooled against 0.326 with the plate term, but its
interval closes from 0.219--0.703 to 0.302--0.351 --- from a factor of 3.2
between the bounds to a factor of 1.2. The N(S)EC changes instead, from 0.198
mg/L (0.054--0.538) to 0.698 (0.144--1.162). A reader who took the pooled fit
at face value would report a no-effect concentration under a third of the one
the plate term gives, with an EC10 interval far wider than the data support.

The weights account for it. Pooled, they spread over `ecxll5` at 0.57, `ecxwb1`
at 0.32, `ecxhormebc4` at 0.05 and `ecxll3` at 0.04; with the plate term
`ecxwb1` takes all of it. `nec3param`, `nec4param`, `nechorme` and `nechorme4`
--- every equation that estimates a threshold --- take no weight in either
fit. A term on the fitted value rescales
...
The three estimates are ordered as they should be --- 0.221, 0.480 and 2.004
µg/L --- and their upper bounds are not. The N(S)EC reaches 2.088 and the EC10
2.133, both above the EC50 median, while the EC50 interval is the narrowest of
the three at 1.794 to 2.765.

A model-averaged estimate is a mixture, and the median and the tail of a mixture
can be set by different components. Two equations hold 0.84 of the weight here
...
averaged median of 0.221 is theirs. The averaged upper bound is not. `nec4param`
places a threshold at 2.106 (1.821--2.357) on a weight of 0.041, `ecxwb2` at
1.620 on 0.049 and `nec3param` at 1.654 on 0.006. Those three hold about a tenth
of the weight between them and sit an order of magnitude above the other twelve,
which is enough to put the 97.5th percentile of the mixture at 2.088.
...
fitted scale, the N(S)EC runs from irgarol at 0.135 µg/L (0.108--0.174) and
...
Every panel declines and the fitted band follows the observations down in each,
with the N(S)EC rule at or a little before the concentration where the points
leave the control level. The response axis is shared and the concentration axis
...
µg/L to tebuthiuron at 10.591, and two things the pairwise probabilities do not
say are visible in the densities.
