...
in a white 96-well plate, at a cell density @luter2025 selected to keep control
...
than an accident. And the plate reader sets its gain per read, which is where the
between-plate differences come from.
...
time, with the wells of one plate joined at their concentration means.
...
Fitting
each plate separately on the normalised response, @luter2025 report EC50s
averaging 0.109 mg/L for copper at 15 minutes (standard deviation 0.036, range
0.069 to 0.201 over sixteen plates) and 0.085 at 30 minutes; for zinc, 1.546 at
15 minutes (0.284, 1.207 to 2.076 over seventeen plates) and 1.013 at 30
minutes. The spread across plates is what a per-plate analysis leaves as
...
would let a concentration-group term absorb plate structure as well.
...
#>                waic   wi
#> nec3param    980.84 0.16
#> nec4param   1070.68 0.00
#> nechorme    1070.18 0.00
#> nechorme4   1039.43 0.00
#> ecxexp       981.74 0.12
#> ecx4param    982.32 0.10
#> ecxwb1       981.19 0.21
#> ecxwb2      1039.73 0.00
#> ecxwb1p3     987.65 0.02
#> ecxwb2p3    1039.54 0.00
#> ecxll5      1040.16 0.00
#> ecxll4      1069.81 0.00
#> ecxll3       980.63 0.20
#> ecxhormebc4  980.66 0.19
#> 
#> 
...
#>        Estimate  Q2.5 Q97.5
#> N(S)EC    -0.03 -1.90  0.58
#> 
#> 
#> Bayesian R2 estimates:
#>             Estimate Est.Error Q2.5 Q97.5
#> nec3param       0.98      0.00 0.97  0.99
#> nec4param       0.70      0.29 0.27  0.98
#> nechorme        0.70      0.29 0.34  0.98
#> nechorme4       0.84      0.25 0.36  0.99
#> ecxexp          0.98      0.00 0.97  0.99
#> ecx4param       0.98      0.00 0.97  0.99
#> ecxwb1          0.98      0.00 0.97  0.99
#> ecxwb2          0.84      0.25 0.36  0.99
#> ecxwb1p3        0.98      0.00 0.98  0.99
#> ecxwb2p3        0.84      0.25 0.36  0.99
#> ecxll5          0.84      0.25 0.36  0.99
#> ecxll4          0.70      0.29 0.34  0.98
...
#>               fit       Q50       Q2.5     Q97.5
#> 1       ungrouped 0.2775346 0.20609842 0.3946245
#> 2 ogl(conc_group) 0.7707091 0.01476189 1.8154879
```
...
#> ecx4param   19314.85 0.01
#> ecxwb1      19301.93 0.32
#> ecxwb2      19345.83 0.00
#> ecxwb1p3    19378.00 0.00
#> ecxwb2p3    19341.98 0.00
#> ecxll5      19299.63 0.57
#> ecxll4      19314.77 0.01
#> ecxll3      19311.92 0.04
#> ecxhormebc4 19311.89 0.05
#> 
#> 
#> Summary of weighted N(S)EC posterior estimates:
#> NB: Model set contains a combination of ECx and NEC
...
#>          fit       Q50      Q2.5     Q97.5
#> 1     pooled 0.3275321 0.2189825 0.7030623
#> 2 ogl(plate) 0.3256430 0.3018052 0.3509309
```
...
follow from what the term has to explain. A concentration group sits at one concentration,
...
given under *Within-concentration grouping*, that zinc rises above its control at
low concentrations.
...
Copper's median is 0.075 mg/L over an interval of 0.043 to 0.092, a factor of
2.1 from end to end. Zinc's is 0.771 over 0.227 to 1.243, a factor of 5.5.
Copper is both the more potent metal and the better determined one on these
data. The single pairwise comparison is 0.0138, which is what that separation
reduces to.
...
the grey scale at day 14, against 90 to 128 at every lower concentration, while
the day-0 values run from 91 to 103 throughout --- so those fragments ended
white. @flores2021 record that all corals at 29 µg/L had died by day 10. The
...
#> 1     NSEC 0.2206198 0.1221051 2.088413
#> 2     EC10 0.4796715 0.4018009 2.133392
#> 3     EC50 2.0036769 1.7936610 2.764714
```
...
the three at 1.804 to 2.735.

A model-averaged estimate is a mixture, and the median and the tail of a
mixture can be set by different components. Two equations hold 0.87 of the
weight here and both place the threshold low: `ecxhormebc4` at 0.226
(0.124--0.292) and `ecxwb1` at 0.179 (0.115--0.230). Neither has an upper bound
above 0.3, and the averaged median of 0.217 is theirs. The averaged upper bound
is not. `nec4param` places a threshold at 2.107 (1.815--2.372) on a weight of
0.039, `ecxwb2` at 1.615 on 0.026 and `nec3param` at 1.657 on 0.007. Those
three hold seven per cent of the weight between them, which is enough to reach
the 97.5th percentile of the mixture: the `nec4param` estimate and the averaged
upper bound of 2.112 agree to three decimal places.

The same draws are visible in the figure below. The credible band steps down at
about 2 µg/L, its upper edge falling from about 0.58 to about 0.31, because the
equations that place a threshold there hold the curve near the control until it
and drop afterwards. An equation given four per cent of the weight has no
effect on the median and still decides where the interval ends, which is a
reason to read a model-averaged interval against the weights rather than on its
own.
...
each, and `ecx4param` returned an R-hat of 1.009 --- inside every conventional
cutoff, and within 0.005 of the two three-parameter equations. At the default
...
orders of magnitude across the seven. Yield reaches half its lowest-dose mean by
1 µg/L for irgarol and 3 µg/L for diuron and ametryn, against 100 µg/L for
...
#> Model weights (Method: pseudobma_bb_weights):
#>              waic   wi
#> nec3param -450.87 0.02
#> nec4param -456.49 0.03
#> ecxexp    -320.72 0.00
#> ecx4param -465.19 0.31
#> ecxwb1    -443.88 0.01
#> ecxwb2    -461.31 0.08
#> ecxwb1p3  -323.19 0.00
#> ecxwb2p3  -446.96 0.01
#> ecxll5    -464.41 0.23
#> ecxll4    -465.13 0.30
#> ecxll3    -432.03 0.00
#> 
...
#> Model weights (Method: pseudobma_bb_weights):
#>              waic   wi
#> nec3param -307.71 0.00
#> nec4param -304.54 0.00
#> ecxexp    -130.23 0.00
#> ecx4param -332.58 0.14
#> ecxwb1    -335.46 0.45
#> ecxwb2    -318.41 0.00
#> ecxwb1p3  -260.54 0.00
#> ecxwb2p3  -313.92 0.00
#> ecxll5    -334.74 0.27
#> ecxll4    -332.64 0.14
#> ecxll3    -302.46 0.00
#> 
...
#> Model weights (Method: pseudobma_bb_weights):
#>              waic   wi
#> nec3param -311.87 0.00
#> nec4param -307.74 0.00
#> ecxexp    -157.09 0.00
#> ecx4param -365.36 0.24
#> ecxwb1    -361.50 0.12
#> ecxwb2    -340.16 0.00
#> ecxwb1p3  -299.96 0.00
#> ecxwb2p3  -343.49 0.00
#> ecxll5    -366.15 0.35
#> ecxll4    -364.96 0.20
#> ecxll3    -357.54 0.09
#> 
...
#> Model weights (Method: pseudobma_bb_weights):
#>              waic   wi
#> nec3param -271.29 0.00
#> nec4param -266.97 0.00
#> ecxexp     -96.87 0.00
#> ecx4param -303.90 0.12
#> ecxwb1    -285.35 0.00
#> ecxwb2    -293.05 0.02
#> ecxwb1p3  -272.01 0.01
#> ecxwb2p3  -298.04 0.08
#> ecxll5    -300.14 0.02
#> ecxll4    -304.03 0.13
#> ecxll3    -308.65 0.62
#> 
...
#> Model weights (Method: pseudobma_bb_weights):
#>              waic   wi
#> nec3param -358.45 0.00
#> nec4param -357.98 0.00
#> ecxexp    -282.53 0.00
#> ecx4param -378.04 0.26
#> ecxwb1    -375.98 0.24
#> ecxwb2    -370.79 0.03
#> ecxwb1p3  -249.30 0.00
#> ecxwb2p3  -360.15 0.00
#> ecxll5    -377.79 0.20
#> ecxll4    -378.13 0.27
#> ecxll3    -351.90 0.00
#> 
...
#> Model weights (Method: pseudobma_bb_weights):
#>              waic   wi
#> nec3param -273.81 0.00
#> nec4param -270.47 0.00
#> ecxexp     -97.48 0.00
#> ecx4param -304.31 0.18
#> ecxwb1    -302.98 0.10
#> ecxwb2    -294.58 0.02
#> ecxwb1p3  -294.57 0.02
#> ecxwb2p3  -298.07 0.05
#> ecxll5    -304.05 0.16
#> ecxll4    -304.33 0.19
#> ecxll3    -304.78 0.28
#> 
...
#> Model weights (Method: pseudobma_bb_weights):
#>              waic   wi
#> nec3param -490.49 0.02
#> nec4param -486.06 0.00
#> ecxexp    -111.06 0.00
#> ecx4param -500.22 0.00
#> ecxwb1    -455.99 0.00
#> ecxwb2    -518.62 0.16
#> ecxwb1p3  -385.98 0.00
#> ecxwb2p3  -520.94 0.50
#> ecxll5    -519.25 0.31
#> ecxll4    -500.48 0.00
#> ecxll3    -479.60 0.00
#> 
...
The 21 pairwise probabilities are a summary of seven posteriors, and those are
in `posterior_data`:
...
This reproduces Figure 11 of @fisher2024. The seven run from irgarol at 0.135
µg/L to tebuthiuron at 10.542, and three things the pairwise probabilities do
not say are visible in the densities.

The three most potent overlap almost entirely. Irgarol at 0.135 and diuron at
0.152 have intervals of 0.107 to 0.175 and 0.109 to 0.195, so one sits almost
inside the other; that is what a `prob_diff` of 0.724 looks like as a
distribution. Ametryn at 0.199 overlaps both.

The posteriors differ in width as much as in position. From end to end,
irgarol's interval spans a factor of 1.6 and diuron's 1.8, against 11.3 for
simazine and 7.5 for atrazine. Two herbicides an order of magnitude apart in
...
@jones2003 report EC50s for the same seven: irgarol 0.7 µg/L, ametryn 1.7,
diuron 2.3, hexazinone 8.8, atrazine 45, simazine 150 and tebuthiuron 175. A
threshold is a lower quantity than a half-effect concentration and these are
lower throughout, by a factor of five for irgarol and seventeen for tebuthiuron.
What survives between the two is the ordering, which agrees except that the
N(S)EC puts diuron below ametryn where the EC50 puts it above --- and those two
are 0.6 µg/L apart in the published EC50s, which is the smallest gap in the
series.
...
overlap over most of their length.
...
to match Figure 11 of @fisher2024, and the parameter-count contrast of
case study 2, which names two equations on each side because the contrast is
the point. No call names `iter`, `warmup` or `control`, so `bnec()`'s defaults
