<!-- NUMBERS TO VERIFY against the production render before this ships. Every
     figure quoted in prose currently comes from the shape-check render (job
     910761, 2 chains, iter = 400), not from a production run:
       s4  N(S)EC 0.17 and 1.19; EC10 0.280 and 0.978; sd 0.165 (0.095-0.263)
       s5  EC10 0.327/0.349 and N(S)EC 0.198/0.771 are from job 902840, which
           predates the 221-commit merge of dev; annotated N(S)EC 0.19 and 0.65
           and the control range 250,000-1,450,000 are from 910761
       s6  divergences 0, 0, 9, 66; R-hat 1.027, 1.029, 1.844, 1.128
       s7  prob_diff values and the per-level N(S)EC are from 902840
     The 0.8 adapt_delta measurement in the provenance section is from the
     exploratory runs and is independent of the render. -->
...
They answer different questions and suit different data. This vignette works
through both, and then through the case where they have to be combined.
...
#> 
#> 
#> 1 model(s) failed to fit: ecxhormebc5
...
<img src="vignette-fig-unnamed-chunk-25-1.png" alt="plot of chunk unnamed-chunk-25" width="60%" />
...
<div class="figure" style="text-align: center">
<img src="vignette-fig-unnamed-chunk-26-1.png" alt="plot of chunk unnamed-chunk-26" width="60%" />
<p class="caption">plot of chunk unnamed-chunk-26</p>
</div>

The term changes the fitted curve, not only its uncertainty. Ungrouped, the
model-averaged curve falls from the control immediately and the annotated
N(S)EC is 0.17 mg/L on a narrow band; with `ogl(conc_group)` it holds near the
control to about 1 mg/L before falling, and the N(S)EC is 1.19 on a band that
spans most of the response range through the transition. EC10 changes with it,
from 0.280 to 0.978 mg/L.
...
<div class="figure" style="text-align: center">
<img src="vignette-fig-unnamed-chunk-30-1.png" alt="plot of chunk unnamed-chunk-30" width="60%" />
...
<div class="figure" style="text-align: center">
<img src="vignette-fig-unnamed-chunk-31-1.png" alt="plot of chunk unnamed-chunk-31" width="60%" />
...
plates, and the pooled fit has nowhere to put that except the residual, which is
why its curve falls from the control with a narrow band and an annotated N(S)EC
of 0.19 mg/L. With `ogl(plate)` the same spread is attributed to the plates and
the population curve holds nearer the control before falling, with an N(S)EC of
0.65.
...
interval narrows from 0.219--0.712 to 0.314--0.382, about sevenfold. The
N(S)EC changes instead, from 0.198 mg/L to 0.771, on intervals that stay wide in
both. A reader who took the pooled fit at face value would report a no-effect
concentration a quarter of the one the plate term gives, with an EC10 interval
seven times wider than the data support.

The weights account for it. Pooled, they spread over `ecxll5` at 0.53, `ecxwb1`
at 0.34, and `ecxll3` and `ecxhormebc4` at 0.05 each; with the plate term they
...
well. The failure is a sampler failure rather than an error, so a set fitted
without checking diagnostics returns an answer that looks ordinary.
...
[@flores2021]. Both are within-concentration designs --- a chamber sits at one
concentration --- and they differ in exactly the way this section needs.
...
``` r
fit_pam <- bnec(yield ~ crf(diuron, "all") + ogl(chamber),
...
#> Model weights (Method: pseudobma_bb_weights):
#>                   waic   wi
#> nec3param     -1657.19 0.00
#> nec4param     -1659.53 0.03
#> nechorme      -1698.00 0.21
#> nechorme4     -1698.11 0.22
#> necsigm       -1654.81 0.00
#> nechormepwr01 -1699.64 0.47
#> ecxexp        -1658.17 0.00
#> ecxsigm       -1660.32 0.00
#> ecx4param     -1646.72 0.01
#> ecxwb1        -1639.31 0.00
#> ecxwb2        -1661.98 0.06
#> ecxwb1p3      -1661.73 0.00
#> ecxwb2p3      -1660.50 0.00
#> ecxll5        -1655.35 0.00
#> ecxll4        -1646.95 0.00
#> ecxll3        -1660.87 0.00
#> ecxhormebc4   -1681.63 0.00
#> ecxhormebc5   -1679.62 0.00
#> 
#> 
#> Summary of weighted N(S)EC posterior estimates:
#> NB: Model set contains a combination of ECx and NEC
...
#> nechorme4         0.98      0.00 0.98  0.99
#> necsigm           0.98      0.00 0.98  0.98
#> nechormepwr01     0.98      0.00 0.98  0.99
#> ecxexp            0.98      0.00 0.98  0.98
#> ecxsigm           0.98      0.00 0.98  0.98
#> ecx4param         0.98      0.00 0.98  0.98
...
#> ecxhormebc4       0.98      0.00 0.98  0.98
#> ecxhormebc5       0.98      0.00 0.98  0.98
```
...
<div class="figure" style="text-align: center">
<img src="vignette-fig-unnamed-chunk-34-1.png" alt="plot of chunk unnamed-chunk-34" width="60%" />
...
``` r
three_par <- bnec(proportion ~ crf(diuron, c("ecxll3", "ecxwb1p3")) + ogl(chamber),
                  data = coral_colour, family = Beta(link = "identity"),
                  chains = 4, iter = 5000, warmup = 4000, seed = 228, refresh = 0,
                  control = list(adapt_delta = 0.99))

four_par <- bnec(proportion ~ crf(diuron, c("nec4param", "ecx4param")) + ogl(chamber),
                 data = coral_colour, family = Beta(link = "identity"),
                 chains = 4, iter = 5000, warmup = 4000, seed = 228, refresh = 0,
                 control = list(adapt_delta = 0.99))
```
...
#> 1 three parameters    ecxll3         0   1.0042
#> 2 three parameters  ecxwb1p3         0   1.0040
#> 3  four parameters nec4param        71   1.0177
#> 4  four parameters ecx4param       243   1.0130
```

The two fits are drawn below, three parameters first.
...
<div class="figure" style="text-align: center">
<img src="vignette-fig-unnamed-chunk-37-1.png" alt="plot of chunk unnamed-chunk-37" width="60%" />
...
<div class="figure" style="text-align: center">
<img src="vignette-fig-unnamed-chunk-38-1.png" alt="plot of chunk unnamed-chunk-38" width="60%" />
<p class="caption">plot of chunk unnamed-chunk-38</p>
</div>

The diagnostics and the figures agree. Neither three-parameter equation records
a divergent transition, and their maximum R-hat is 1.027 and 1.029; `nec4param`
records 9 divergences at an R-hat of 1.844 and `ecx4param` 66 at 1.128. The
four-parameter figure shows what an R-hat of 1.8 looks like: its credible band
covers almost the whole response range from below 0.1 to near 0.7 across most of
the concentration axis, so the fit states almost nothing about the curve while
still drawing a mean through the data.

Five concentrations is the conventional minimum for a concentration-response
design, and it is enough to place a midpoint or a threshold but not enough to
place both asymptotes as well. The design does not announce that: every equation
is offered, every one returns an object, and only the diagnostics and the width
of the band say which of them the data can support. #301 proposes restricting a
set by the number of curve parameters for this reason.

One further point belongs here. The colour score has exactly one observation on
...
<div class="figure" style="text-align: center">
<img src="vignette-fig-unnamed-chunk-41-1.png" alt="plot of chunk unnamed-chunk-41" width="60%" />
...
they are levels rather than a grouping, and each is fitted with the structure
sections 3 to 5 arrived at: the censored response, a plate term for the gain, and
a dispersion sub-model.
...
<div class="figure" style="text-align: center">
<img src="vignette-fig-unnamed-chunk-45-1.png" alt="plot of chunk unnamed-chunk-45" width="60%" />
...
fitted scale, the N(S)EC runs from irgarol at 0.135 and diuron at 0.152 µg/L to
simazine at 4.31 and tebuthiuron at 10.6, with ametryn, hexazinone and atrazine
between them. `prob_diff` gives the probability that the first level of each pair
has the higher estimate, so a value near 0 or 1 separates two herbicides and one
near 0.5 does not: ametryn against tebuthiuron is 0.000 and atrazine against
diuron 0.998, while simazine against tebuthiuron at 0.062 is the least separated
of the 21 pairs. For the metals the single comparison is 0.003, so copper and
zinc are distinguished on these data.
...
`ecxhormebc5` is reported as not fitted in every `lum31` fit. It fails to
initialise rather than to sample: its hormesis term `exp(slope) * x` is negative
wherever the predictor is, which on `log(conc)` is 64 per cent of the
observations, and a starting value drawn from the prior puts the mean below zero,
which a Gamma with an identity link cannot represent. The same equation fits
without difficulty on `coral_pam`, whose predictor is never negative. See #344.
