...
#' coefficient applies to that scalar. This is not true after extending a
#' parameter's right-hand side in a formula passed directly to
#' \code{\link[brms]{brm}}. For example, after changing \code{top ~ 1} to
#' \code{top ~ treatment}, the same prior applies to both the intercept and the
#' treatment contrasts.
#'
#' Rebuild the priors against the target \pkg{brms} formula rather than reusing
#' this object unchanged. Adding \code{coef = "Intercept"} is not a general
#' conversion: \pkg{brms} does not allow \code{coef} together with
#' \code{lb} or \code{ub}, and those bounds are part of the priors
#' \code{bayesnec} generates for several parameters. Use
#' \code{\link[brms]{get_prior}} to inspect the target coefficients, then set a
#' prior for each coefficient under the parameterisation used by that formula.
#'
