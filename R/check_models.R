...
#'
#' That is the sharpest case rather than the whole reason. It is a statement
#' about a predictor reaching above 1, while the exclusion is unconditional on
#' the data, and what justifies that is the exponent.
#' \code{1 / (1 + exp(slope))} tends to 0 as \code{slope} grows, so
#' \code{x^(1 / (1 + exp(slope)))} tends to 1 for \emph{every} \code{x} above
#' 0, however small. Below the threshold the mean is then \code{top + 1} in the
#' limit, so for any \code{top} above 0 there is a \code{slope} at which the
#' mean exceeds 1, on any predictor and at any concentration, and at ordinary
#' values of \code{slope} under its \code{normal(0, 5)} prior rather than at
#' tail values.
#'
...
#' shrink it. Bounding \code{nec} below 1 is therefore not the fix it appears to
#' be. A low \code{nec} is necessary and nowhere near sufficient, because the
#' admissible set is a curved region in \code{top}, \code{slope} and \code{nec}
#' jointly rather than a bound on \code{nec} alone, and the majority of that
#' joint space still puts the mean above 1. The initial-value search finding an
#' admissible draw does not make the model usable. See #177 and #302.
#'
#' @return A \code{\link[base]{character}} vector.
#'
#' @noRd
