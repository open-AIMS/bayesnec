...
#' precise.
#' @param posterior A \code{\link[base]{logical}} value indicating if the full posterior sample of calculated nsec values should be returned 
...
#' @param prob_vals A vector indicating the probability values over which to return the estimated nsec value. Defaults to 0.5 (median) and 0.025 and 0.975 (95 percent credible intervals). 
#'
#' @details 
#' For \code{hormesis_def}, if "max", then nsec values are calculated as a decline from the maximum estimates (i.e. the peak at nec);
...
#' 
#' @return A vector containing the estimated nsec value, including upper and lower 95% credible interval bounds.
#' 
...
nsec.default <- function(object, sig_val = 0.01, precision = 1000,
  ...
  if (grepl("horme", object$model)) {
    ...
    nec_posterior <- unlist(posterior_samples(object$fit, pars = "nec_Intercept"))  
    if (hormesis_def == "max") {
      reference <- quantile(apply(pred_vals$posterior, 2, max), probs = sig_val)   
    }
  }
...
nsec <- function(object, sig_val = 0.01, precision = 1000,
                posterior = FALSE, hormesis_def = "control", x_range = NA,
                xform = NA, prob_vals = c(0.5, 0.025, 0.975)) {
  UseMethod("nsec")
}
...
nsec.bayesmanecfit <- function(object, sig_val = 0.01, precision = 1000,
  ...
  sample_nsec <- function(x, object, sig_val, precision,
    ...
    out <- nsec.default(object$mod_fits[[x]], sig_val = sig_val,
                       precision = precision, posterior = posterior, hormesis_def = hormesis_def,
