...
#' The full \code{n_draws x resolution} matrix those summaries came from is not
#' stored, because it dominated the size of a fitted object; it is recomputed
#' from \code{fit} where it is needed. To obtain it directly, use
#' \code{posterior_epred(x, newdata = bnec_newdata(x, resolution = 1000),
#' re_formula = NA)}. An object saved by an earlier version may still hold it as
#' a \code{posterior} element, and is unaffected.
