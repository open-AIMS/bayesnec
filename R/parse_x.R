...
parse_x <- function(x, y, data, x_var, y_var, model, trials_var, family) {
  if (!is(x, "formula")) {
    ...
    ylabel <- if (!is.null(y)) deparse1(substitute(y))
    xy <- xy.coords(x, y, xlabel, ylabel, log = "")
    data <- data.frame(xy$x, xy$y)
    if (nrow(data) == 0) {
      stop("Your x input data contains no rows.")
    }
    x_var <- xy$xlab 
    y_var <- xy$ylab
    ...
    if (missing(trials_var)) {
      ...
      if (!is.na(trials_var)) {
        if (max(data[, y_var]) > 1) {
          ...
          warning("Argument trials_var supplied, implying a binomial family,",
                  " y data treated as integer.")
        }
        if (is(trials_var, "numeric")) {
          if (length(trials_var) == nrow(data)) {
            ...
            if (max(data[, y_var]) <= 1) {
              ...
              warning("The maximum observed value of the response was < 1 and",
                      " assumed to be a proportion of trials_var.")
            }
          } else {
    ...
    if (!missing(x_var) || !missing(y_var) || !missing(model) ||
          !is.na(trials_var)) {
      warning("Arguments x_var, y_var, trials_var and model ignored in the",
              " case of formula syntax.")
    }
    get_vals <- gsub("model", "", unlist(strsplit(as.character(x), ",")))
    if (grepl("|", get_vals[2], fixed = TRUE)) {
      inputs <- gsub("[^\\w]*", "", unlist(strsplit(get_vals, " | trials")),
                     perl = TRUE)
      y_var <- inputs[2]
      x_var <- inputs[5]
      model <- inputs[7]
