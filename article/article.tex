...
\code{nechorme4pwr} is five parameter decay model with an NEC step function equivalent to \code{nec4param} with the addition of a power increase prior to \(\eta\), given by the equation:
...
with the following \code{brmsformula}: \code{y \textasciitilde{} (1/(1 + ((1/top) - 1) * exp(-exp(slope) * x))) * exp(-exp(beta) * (x - nec) * step(x - nec))}. The \code{nechormepwr01} model is a \emph{hormesis} model \citep{Mattson2008}, allowing an initial increase in the response variable at concentrations below \(\eta\). The model is 0-bounded, thus not suitable for Gaussian response data or the use of a \code{"logit"} or \code{"log"} link function. In this case the linear version (\code{neclinhorme}) should be used.
