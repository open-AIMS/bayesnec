...
\author{by Rebecca Fisher, Diego R. Barneche, Gerard Ricardo, and David Fox}

\maketitle

\abstract{%
The \pkg{bayesnec} package has been developed for R to fit
...
effect-concentration (of specified percentage `x',
\emph{EC\textsubscript{x}}) thresholds from non-linear models fitted
using Bayesian Hamiltonian Monte Carlo (HMC) via \pkg{brms}
\citep{Burkner2017, Burkner2018} and \pkg{rstan} \citep{rstan2021}. In
\pkg{bayesnec} it is possible to fit a single model, custom models-set,
specific model-set or all of the available models. When multiple models
are specified the \texttt{bnec} function returns a model weighted
average estimate of predicted posterior values. A range of support
functions and methods are also included to work with the returned
single, or multi-model objects that allow extraction of raw, or model
averaged predicted, \emph{NEC}, \emph{NSEC} and
\emph{EC\textsubscript{x}} values and to interrogate the fitted model or
...
In qualitative terms, C-R models are typically of a decaying shape,
...
in \pkg{bayesnec}}} for the mathematical definition of each model):
...
list a few), \pkg{nlme} \citep{pinheiro2021}, \pkg{lme4}
\citep{pinheiro2021}, \pkg{rjags} \citep{Su2015} and \pkg{rstan}
\citep{rstan2021}.
...
The \pkg{drc} package \citep{Ritz2016} was developed as a user friendly
frequentist based solution to C-R modelling in R, and is currently
widely used across a range of disciplines. \pkg{drc} implements a broad
...
comparisons between \pkg{drc} and \pkg{bayesnec} standard output using
...
such as \pkg{R2WinBUGS}, \pkg{R2jags} \citep{Su2015} and \pkg{rstan}
...
chief among which is \pkg{brms} \citep{Burkner2017}, that allows a broad
range of models to be easily fitted using \pkg{rstan} through simpler
\pkg{lme4}-like formula syntax. However, even with packages like
...
\pkg{bayesnec} version 2.0.2 (available on CRAN) builds upon an
implementation of the \emph{NEC} model described in
\citep{Fox2010, Pires2002}. The \pkg{bayesnec} package provides an
...
In many cases, the denominator (a.k.a. ``evidence'' or ``marginal'') is
analytically intractable, and therefore we resort to algorithmic
approximations such as Markov Chain Monte Carlo (MCMC). \pkg{bayesnec}
uses the extensive functionality of the \pkg{brms} package to write
...
\citep{hoffman2014}. \pkg{bayesnec} first takes the input model or
...
calls \pkg{brms} \citep{Burkner2017, Burkner2018} to generate the Stan
program code and fit the model through \pkg{rstan} \citep{rstan2021}. In
doing so, \pkg{bayesnec} (as of version 2.0.2) allows for response
...
Beta and Beta-binomial. Future implementations of \pkg{bayesnec} might
include additional distributions which are currently only implemented in
\pkg{brms}. In addition to greater flexibility in the available response
distributions, \pkg{bayesnec} includes a larger set of 23 \emph{NEC}-
...
\pkg{bayesnec}}{Models in }}\label{mdbnc}}
...
\(\eta = \text{NEC}\), the No-Effect-Concentration value (the
...
The \textbf{ecxexp} model is a basic exponential decay model, given by
...
The \textbf{ecxwb1} model is a 4-parameter sigmoidal decay model which
...
While very similar to the \textbf{ecxwb1} \citep[according
...
The \textbf{ecxll4} model is a 4-parameter sigmoidal log-logistic decay
...
The \textbf{nec4param} model is a 3-parameter decay model with the
...
The \textbf{nechorme4} model is 4 parameter decay model with an
...
The \textbf{nechormepwr01} model is a basic exponential decay model with
an \emph{NEC} step function equivalent to \textbf{nec3param}, with the
addition of a power increase prior to \(\eta\), given by the equation:
...
``weakly'' informative priors for fitting models in \pkg{bayesnec}. The
...
model averaging within the \pkg{bayesnec} package is to properly capture
...
that reason, by default \pkg{bayesnec} uses the ``pseudobma'' method.

\hypertarget{usage}{%
\section{Usage}\label{usage}}

\hypertarget{the-fitting-function-bnec}{%
...
\pkg{brms} (see the `aterms' section of the \texttt{brms::brm}'s help
...
the input formula can either be a character string or a validated object
...
section \protect\hyperlink{mdbnc}{Models in \pkg{bayesnec}}).
...
Models fitted by \texttt{bnec} will invariably inherit a class
...
done via either \pkg{base} R (\texttt{plot}) and \pkg{ggplot2}
...
\caption{\pkg{ggplot2} `autoplot` of the example fit. The solid black line is the fitted median of the posterior prediction, dashed black lines are the 95\% credible intervals, and the red vertical lines show the estimated \textit{NEC} value.}\label{fig:base-plot}
\end{figure}
\begin{Sinput}
# or alternatively
...
The current version of \pkg{bayesnec} harnesses the powerful modelling
flexibility of \pkg{brms} for accommodating hierarchical designs and
...
used to generate the underlying \texttt{brmsformula} for the \pkg{brms}
...
Examples of how to implement hierarchical effects in \pkg{bayesnec} can
be found on the help file of function \texttt{bnf}.

\hypertarget{bnchmrk}{%
\section{Existing alternatives}\label{bnchmrk}}

\pkg{bayesnec} is built upon the precursor R package \pkg{jagsNEC},
which writes and fits C-R models in JAGS \citep{Plummer2003}.
\pkg{bayesnec} was then expanded to include several C-R models and
...
The main existing tool in R is the frequentist-based package \pkg{drc}
...
{\centering \includegraphics{article_files/figure-latex/drccomparison-1} 
...
example). As the model-set in \pkg{bayesnec} may change through time it
...
\hypertarget{model-suitability-for-nec-and-ecx-estimation}{%
...
\emph{EC\textsubscript{x}} estimates can be equally obtained from both
...
\pkg{bayesnec}, with a reasonable fit achieved through a combination of
the appropriate distribution being applied to the response, and
\pkg{bayesnec}'s \texttt{make\_inits} function which ensures initial
values passed to \pkg{brms} yield response values within the range of
...
The Indian Ocean Marine Research Centre,University of Western
Australia,Crawley, WA, Australia\\%
\\
%
%
\textit{ORCiD: \href{https://orcid.org/0000-0001-5148-6731}{0000-0001-5148-6731}}\\%
...
%
%
\textit{ORCiD: \href{https://orcid.org/0000-0002-4568-2362}{0000-0002-4568-2362}}\\%
%
}
...
%
%
\textit{ORCiD: \href{https://orcid.org/0000-0002-7761-0806}{0000-0002-7761-0806}}\\%
%
}
...
%
%
\textit{ORCiD: \href{https://orcid.org/0000-0002-3178-7243}{0000-0002-3178-7243}}\\%
%
}
