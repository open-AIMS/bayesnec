...
with the following \code{brmsformula}: \code{y \textasciitilde{} top * exp(-exp(beta) * x)}. The model is 0-bounded, thus not suitable for Gaussian response data or the use of a \code{"logit"} or \code{"log"} link function.

\code{ecxsigm} is a simple sigmoidal decay model, given by the equation:
\(y_i = \tau e^{-e^{\beta} x_i^{e^\epsilon}}\)
with the following \code{brmsformula}: \code{y \textasciitilde{} top * exp(-exp(beta) * x\^{}exp(d))}. The model is 0-bounded, thus not suitable for Gaussian response data or the use of a \code{"logit"} or \code{"log"} link function.

\code{ecx4param} is a 4-parameter sigmoidal decay model, given by the equation:
\(y_i = \tau + (\delta - \tau)/(1 + e^{e^{\beta} (\omega - x_i)})\)
with the following \code{brmsformula}: \code{y \textasciitilde{} top + (bot - top)/(1 + exp((ec50 - x) * exp(beta)))}.
...
with the following \code{brmsformula}: \code{y \textasciitilde{} bot + (top - bot) * exp(-exp(exp(beta) * (x - ec50)))}.
...
with the following \code{brmsformula}: \code{y \textasciitilde{} 0 + (top - 0) * exp(-exp(exp(beta) * (x - ec50)))}. The model is 0-bounded, thus not suitable for Gaussian response data or the use of a \code{"logit"} or \code{"log"} link function.
...
with the following \code{brmsformula}: \code{y \textasciitilde{} bot + (top - bot) * (1 - exp(-exp(-exp(beta) * (x - ec50))))}. While very similar to the \code{ecxwb1} \citep[according to][]{Ritz2016}, fitted \code{ecxwb1} and \code{ecxwb2} models can differ slightly.
...
with the following \code{brmsformula}: \code{y \textasciitilde{} 0 + (top - 0) * (1 - exp(-exp(-exp(beta) * (x - ec50))))}. The model is 0-bounded, thus not suitable for Gaussian response data or the use of a logit or log link function.
...
with the following \code{brmsformula}: \code{y \textasciitilde{} bot + (top - bot)/(1 + exp(exp(beta) * (x - ec50)))\^{}exp(f)}.
...
with the following \code{brmsformula}: \code{y \textasciitilde{} bot + (top - bot)/(1 + exp(exp(beta) * (x - ec50)))}.
...
with the following \code{brmsformula}: \code{y \textasciitilde{} 0 + (top - 0)/(1 + exp(exp(beta) * (x - ec50)))}. The model is 0-bounded, thus not suitable for Gaussian response data or the use of a \code{"logit"} or \code{"log"} link function.
...
with the following \code{brmsformula}: \code{y \textasciitilde{} bot + (top - bot + exp(slope) * x)/(1 + exp(exp(beta) * (x - ec50)))}.
...
with the following \code{brmsformula}: \code{y \textasciitilde{} 0 + (top - 0 + exp(slope) * x)/(1 + exp(exp(beta) * (x - ec50)))}. The model is 0-bounded, thus not suitable for Gaussian response data or the use of a \code{"logit"} or \code{"log"} link function.
...
with the following \code{brmsformula}: \code{y \textasciitilde{} top - exp(slope) * (x - nec) * step(x - nec)}. Because the model contains linear predictors it is not suitable for 0--1 bounded data (Binomial and Beta distributions with \code{"identity"} link). As the model includes a linear decline with concentration, it is also not suitable for 0--\(\infty\) bounded data (Gamma, Poisson, Negative Binomial with \code{"identity"} link).
...
with the following \code{brmsformula}: \code{y \textasciitilde{} bot + (top - bot) * exp(-exp(beta) * (x - nec) * step(x - nec))}.
...
with the following \code{brmsformula}: \code{y \textasciitilde{} (top + x\^{}(1/(1 + exp(slope)))) * exp(-exp(beta) * (x - nec) * step(x - nec))}. Being a \emph{hormesis} model \citep{Mattson2008}, \code{nechormepwr} allows an initial increase in the response variable at concentrations below \(\eta\). The model is 0-bounded, thus not suitable for Gaussian response data or the use of a \code{"logit"} or \code{"log"} link function. Because the model can generate predictions \textgreater{} 1 it should not be used for Binomial and Beta distributions with \code{"identity"} link. In this case the \code{nechromepwr01} model should be used.
...
with the following \code{brmsformula}: \code{y \textasciitilde{} bot + ((top + exp(slope) * x) - bot) * exp(-exp(beta) * (x - nec) * step(x - nec))}. The \code{nechorme4} model is a \emph{hormesis} model \citep{Mattson2008}, allowing an initial increase in the response variable at concentrations below \(\eta\).
...
with the following \code{brmsformula}: \code{y \textasciitilde{} bot + ((top + x\^{}(1/(1 + exp(slope)))) - bot) * exp(-exp(beta) * (x - nec) * step(x - nec))}. Being a \emph{hormesis} model \citep{Mattson2008}, \code{nechorme4pwr} allows an initial power increase in the response variable at concentrations below \(\eta\). Because the model can generate predictions \textgreater{} 1 it should not be used for Binomial and Beta distributions with \code{"identity"} link. In this case the \code{nechromepwr01} model should be used.
...
with the following \code{brmsformula}: \code{y \textasciitilde{} (1/(1 + ((1/top) - 1) * exp(-exp(slope) * x))) * exp(-exp(beta) * (x - nec) * step(x - nec))}. \linebreak Being a \emph{hormesis} model \citep{Mattson2008}, \code{nechormepwr01} allows an initial increase in the response variable at concentrations below \(\eta\). The model is 0-bounded, thus not suitable for Gaussian response data or the use of a \code{"logit"} or \code{"log"} link function. In this case the linear version (\code{neclinhorme}) should be used.
...
The \code{summary} method provides the usual summary of model parameters and any relevant model fit statistics as returned in the underlying \code{brm} model fit(s). In the specific case of a \code{bayesmanecfit} object, the summary includes a list of fitted models, their respective model weights, and a model-averaged no-effect toxicity estimate. Where the fitted model(s) are NEC models (threshold models, containing a step function) the no-effect estimate is a true no-effect-concentration \citep[NEC, see][]{Fox2010}. Where the fitted model(s) are smooth ECx models with no step function, the no-effect estimate is a no-significant-effect-concentration \citep[NSEC, see][]{Fisher2023}. In the case of a \code{bayesmanecfit} that contains a mixture of both NEC and ECx models, the no-effect estimate is a model averaged combination of the NEC and NSEC estimates, and is reported as the N(S)EC \citep[see][]{fisher2023ieam}.
...
By default the plot shows the fitted posterior curve with 95\% credible intervals, along with an estimate of the \(\eta = \text{NEC}\) value. For more examples using \pkg{bayesnec} models for inference see the on-line the vignettes at \url{https://open-aims.github.io/bayesnec/articles/}, as well as Section \ref{modsuit} below.
...
We can plot all the models contained within the \code{bayesmanecfit} using the \texttt{autoplot} function, with all\_models = TRUE (\autoref{fig:fullbayesmanecplotametrynALLplot}):
