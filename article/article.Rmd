...
$y_i = \tau - e^{\alpha} x_i$
with the following \code{brmsformula}: \code{`r as.character(show_params("ecxlin")[[1]][[1]])`}. Because the model contains linear predictors it is not suitable for 0--1 bounded data (i.e., Binomial and Beta families with an \code{"identity"} link function). As the model includes a linear decline with concentration, it is also not suitable for 0 bounded data (Gamma, Poisson, Negative Binomial with an \code{"identity"} link).

\code{ecxexp} is a basic exponential decay model, given by the equation: $y_i = \tau e^{-e^{\beta} x_i}$
with the following \code{brmsformula}: `r as.character(show_params("ecxexp")[[1]][[1]])`. The model is 0-bounded, thus not suitable for Gaussian response data or the use of a \code{"logit"} or \code{"log"} link function.

\code{ecxsigm} is a simple sigmoidal decay model, given by the equation:
$y_i = \tau e^{-e^{\beta} x_i^{e^\epsilon}}$
with the following \code{brmsformula}: `r as.character(show_params("ecxsigm")[[1]][[1]])`. The model is 0-bounded, thus not suitable for Gaussian response data or the use of a \code{"logit"} or \code{"log"} link function.

\code{ecx4param} is a 4-parameter sigmoidal decay model, given by the equation:
$y_i = \tau + (\delta - \tau)/(1 + e^{e^{\beta} (\omega - x_i)})$
with the following \code{brmsformula}: `r as.character(show_params("ecx4param")[[1]][[1]])`
...
with the following \code{brmsformula}: `r as.character(show_params("ecxwb1")[[1]][[1]])`
...
with the following \code{brmsformula}: `r as.character(show_params("ecxwb1p3")[[1]][[1]])`. The model is 0-bounded, thus not suitable for Gaussian response data or the use of a \code{"logit"} or \code{"log"} link function.
...
with the following \code{brmsformula}: `r as.character(show_params("ecxwb2")[[1]][[1]])`. While very similar to the \code{ecxwb1} [according to @Ritz2016], fitted \code{ecxwb1} and \code{ecxwb2} models can differ slightly.
...
with the following \code{brmsformula}: `r as.character(show_params("ecxwb2p3")[[1]][[1]])`. The model is 0-bounded, thus not suitable for Gaussian response data or the use of a logit or log link function.
...
with the following \code{brmsformula}: `r as.character(show_params("ecxll5")[[1]][[1]])`.
...
with the following \code{brmsformula}: `r as.character(show_params("ecxll4")[[1]][[1]])`.
...
with the following \code{brmsformula}: `r as.character(show_params("ecxll3")[[1]][[1]])`. 
...
with the following \code{brmsformula}: `r as.character(show_params("ecxhormebc5")[[1]][[1]])`.
...
with the following \code{brmsformula}: `r as.character(show_params("ecxhormebc4")[[1]][[1]])`. The model is 0-bounded, thus not suitable for Gaussian response data or the use of a \code{"logit"} or \code{"log"} link function.
...
with the following \code{brmsformula}: `r as.character(show_params("neclin")[[1]][[1]])`. Because the model contains linear predictors it is not suitable for 0--1 bounded data (Binomial and Beta distributions with \code{"identity"} link). As the model includes a linear decline with concentration, it is also not suitable for 0--$\infty$ bounded data (Gamma, Poisson, Negative Binomial with \code{"identity"} link).
...
with the following \code{brmsformula}: `r as.character(show_params("nec4param")[[1]][[1]])`.
...
with the following \code{brmsformula}: `r as.character(show_params("nechorme4")[[1]][[1]])`. The \code{nechorme4} model is a *hormesis* model [@Mattson2008], allowing an initial increase in the response variable at concentrations below $\eta$.
...
with the following \code{brmsformula}: `r as.character(show_params("nechorme4pwr")[[1]][[1]])`. The \code{nechorme4pwr} model is a *hormesis* model [@Mattson2008], allowing an initial power increase in the response variable at concentrations below $\eta$. Because the model can generate predictions > 1 it should not be used for Binomial and Beta distributions with \code{"identity"} link. In this case the \code{nechromepwr01} model should be used.
...
with the following \code{brmsformula}: `r as.character(show_params("nechormepwr01")[[1]][[1]])`. The \code{nechormepwr01} model is a *hormesis* model [@Mattson2008], allowing an initial increase in the response variable at concentrations below $\eta$. The model is 0-bounded, thus not suitable for Gaussian response data or the use of a \code{"logit"} or \code{"log"} link function. In this case the linear version (\code{neclinhorme}) should be used.
...
```{r base-plot, fig.height = 3.5, fig.width = 4, fig.cap = "\\pkg{ggplot2} \\code{autoplot} of the example fit. The solid black line is the fitted median of the posterior prediction, dashed black lines are the 95\\% credible intervals, and the vertical lines show the estimated \\textit{NEC} value."}
...
```{r brmsplot, fig.width = 5.14, fig.height = 6, fig.cap = "Default \\pkg{brms} plot of the \\code{nec4param} model showing the posterior densities and chain mixing for each of the included parameters.", dependson="fit"}
...
```{r sampleprior, echo=FALSE, fig.height = 3.5, fig.width = 4.5, fig.cap = "Frequency histograms of samples of the default priors used by \\pkg{bayesnec} for fitting the \\code{nec4param} model to the example data."}
...
```{r checkpriorsingle, echo=FALSE,fig.height = 3.5, fig.width = 6, fig.cap = "A comparison of the prior and posterior parameter probability densities for the \\code{nec4param} model fit to the example data."}
check_priors(fit)
```
...
We can also check the \code{rhat} values of the fitted models using the \code{rhat} function:
...
We can plot all the models contained within the \code{bayesmanecfit} using the `autplot` function, with all_models = TRUE (\autoref{fig:fullbayesmanecplotametrynALLplot}):
...
* `r round(timeallametrynfits["elapsed"]/60,1)` minutes for the full \code{decline} model set fit to the herbicide ametryn in the example herbicide data set (see Section \ref{isingle}); and

* `r round(timeallherbfits["elapsed"]/60,1)` minutes to fit the full \code{decline} model set to all seven herbicides in the example herbicide data set (see Section \ref{iall})

These computing times can be substantially reduced by running the four chains in parallel, by passing the argument \code{cores = 4} via the \code{bnec(...)} call, and will also vary widely among different hardware and operating systems.
...
The data requirements for saving model fits can be relatively large. The single model fit (see \ref{example}) generates an object of `r print(object.size(fit), units="Mb")`; the full \code{decline} model set fit to the herbicide ametryn (see \ref{isingle}) and object of `r print(object.size(manecfit_ametryn), units="Mb")`; and the full \code{decline} model set to all seven herbicides in the example data set (see \ref{iall}) and object of `r print(object.size(manecfit), units="Mb")`.
