...
abstract: >
  ...
  fitted using Bayesian Hamiltonian Monte Carlo (HMC) via \pkg{brms} [@Burkner2017; @Burkner2018] and \pkg{rstan} [@rstan2020]. 
...
Base R (`plot`) and ggplot2 (`autoplot`) plotting methods, as well as predict methods have also been developed for both `bayesnecfit` and `bayesmanecfit` model classes. In addition, there are method-based functions for extracting *ECx* (`ecx`), *NEC* (`nec`) and *NSEC* (`nsec`) threshold values. In all cases the posterior draws that underpin these functions are achieved through `posterior_epred` from the \pkg{brms} package. An example base plot of a `bayesmanecfit` model fit can be seen in \autoref{fig:base-plot}.
