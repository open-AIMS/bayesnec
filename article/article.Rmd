...
```{r fullbayesmanecpreplot, echo = FALSE, results = "hide", cache = TRUE, dependson = "fullbayesmanec"}
good_fits <- purrr::map(manecfit, function(x) {
  ...
  out_fit
  # note for the decline model set there are no fits with
  #   bad rhat values for this example.
})
all_plots <- lapply(good_fits, function(x) {
  nec_vals <- signif(exp(nec(x)), 2)
  nec_lab <- paste(
    "N(S)EC = ", nec_vals[1], " (", nec_vals[2], "-", nec_vals[3], ")",
    sep = ""
  )
  autoplot(x, nec = FALSE) +
    ylim(0, 0.8) +
    xlim(-2.5, 6) +
    geom_text(label = nec_lab, aes(x = -2, y = 0.8), size = 3, hjust = 0) +
...
```{r modelweights}
library(tidyr)
purrr::map_dfr(good_fits, function(x) {
  x$mod_stats %>%
...
figure <- ggpubr::ggarrange(
  plotlist = all_plots, nrow = 4, ncol = 2, labels = herbicides,
  font.label = list(color = "black", size = 12, face = "plain"), align = "hv"
)
ggpubr::annotate_figure(
  figure, left = text_grob("FvFm", rot = 90),
