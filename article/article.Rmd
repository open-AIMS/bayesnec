...
abstract: >
  ...
  *NEC*, *NSEC* and *ECx* values and to interrogate the fitted model or model-set. The statistical methods used 
...
keywords:
  formatted: [ecotoxicology, effect concentration, threshold derivation, "\\textit{NEC}", "\\proglang{brms}"]
...
```{r brms-plot, fig.height=7, fig.cap="Default \\pkg{brms} plot of the \\texttt{nec4param} model showing the posterior probability densities and chain mixing for each of the included parameters.", dependson="fit"}
plot(exmp_fit$mod_fits$nec4param$fit)
```

\newpage
which yields a plot of the posterior densities as well as trace plots of chains for each parameter in the specified model (\autoref{fig:brms-plot}).
...
The `model` argument may also be one of: `all`, meaning all of the available models will be fit; `ecx`, meaning only models excluding the $\eta = \text{NEC}$ step parameter will be fit; or `nec`, meaning only models with the $\eta = \text{NEC}$ step parameter (see below [\textit{Parameter definitions}](#parameter-definitions)) will be fit. There are a range of other pre-defined model groups available. The full list of currently implemented model groups can be seen using the function `models()`.

Please see the [Model details](https://open-aims.github.io/bayesnec/articles/example2b.html) vignette or `?models("all")` for more information on all the models available in \pkg{bayesnec} and their specific formulation. 

### Parameter definitions

Where possible we have aimed for consistency in the interpretable meaning of the individual parameters across models. Across the currently implemented model-sets, models contain from two (basic linear or exponential decay, see e.g. `models("all")[c("ecxlin", "ecxexp")]` to five possible parameters (e.g. `models("all")[c("nechorme4", "ecxhormebc5")]`), including:
 ...
 - $\eta = \text{NEC}$, the no-effect-concentration value (the x concentration value at which the breakpoint in the regression is estimated, see [\textit{Model types for NEC and ECx estimation}](#model-types-for-nec-and-ecx-estimation) and @Fox2010 for more details on parameter based *NEC* estimation);
...
```{r sample-prior, fig.cap="Frequency histograms of samples of the default priors used by bnec for fitting the \\texttt{nec4param} model to the example data.", dependson="fit"}
...
```{r check-priorsingle, fig.cap="A comparison of the prior and posterior parameter probability densities for the \\texttt{nec4param} model fit to the example data.", dependson="fit"}
...
The current version of \pkg{bayesnec} harnesses the powerful mixed (hierarchical) modelling capabilities of \pkg{brms} for accommodating hierarchical designs and other forms of non-independence. This is achieved by allowing a list of  grouping terms to be passed through the argument *random*, which are then used to update the \pkg{brms} model formula. Hierarchical effects can be in the form of an offset, which effectively allows different mean response levels across groups, and is achieved through specifying the term `ost` in the named list. Hierarchical effects can also be added to any or all of the non-linear parameters in the model. Note that implementing hierarchical effects in a non-linear modelling setting is non-trivial and considerable thought and testing should go into selecting an appropriate hierarchical structure.
