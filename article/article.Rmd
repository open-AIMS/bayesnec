...
The `summary` method provides the usual summary of model parameters and any relevant model fit statistics as returned in the underlying `brm` model fit(s). In the specific case of a `bayesmanecfit` object, the summary includes a list of fitted models, their respective model weights, and a model-averaged *NEC*---which is reported with a warning when it contains *NSEC* values (see below section on \ref{modsuit}).
...
By default the plot shows the fitted posterior curve with 95% credible intervals, along with an estimate of the $\eta = \text{NEC}$ value. Please see on-line the [vignettes](https://open-aims.github.io/bayesnec/articles/) for more examples using \pkg{bayesnec} models for inference, as well as section \ref{modsuit} below.
...
```{r drccomparisonplot, echo = FALSE, fig.width = 6, fig.height = 3, fig.cap = "A comparison of the bayesnec and drc model fits and estimated NEC values for the \\texttt{nec3param} model, fit to data on maximum effective quantum yield of symbiotic dinoflagellates (in hospite) in Seriatopora hystrix exposed to elevated hexazinone and tebuthiuron (range 0.3 to 1000 ug per L) for 10 h."}
...
Following model fitting We first use the `rhat` function as one diagnostic test to check that all models fitted successfully. In this case, for all seven datasets and all the models in the `decline` model set, all parameters met the criterion of rhat < 1.05, suggesting relatively good model fits. Other diagnostic procedures include making use of the `check_chains()` and `check_priors()` functions to ensure there is good chain mixing, and that the default priors were suitable, although results from these checks are omitted here for brevity.
...
```{r modelweights, weightsTab, echo=FALSE}
library(tidyr)
modtab <- purrr::map_dfr(good_fits, function(x) {
  x$mod_stats %>%
    dplyr::select(model, wi) %>%
    dplyr::mutate(wi = round(wi, 3))
}, .id = "herbicide") %>%
  tidyr::pivot_wider(names_from = herbicide, values_from = wi)
kableExtra::kable(modtab)
#, caption="Fitted models and their relative pseudo-BMA weights for C-R curves for the effects seven herbicides on maximum effective quantum yield of symbiotic dinoflagellates of the coral Seriatopora hystrix.", format='latex', booktabs=T)
```
