...
Note that strictly positive parameters (for example, $\beta$ and $\epsilon$) have been coded as exponents to allow the use of normal priors and stabilize model fitting.
...
For the parameters $\beta = \text{beta}$, $\alpha = \text{slope}$ and $\epsilon = \text{d}$ we first ensured any relevant transformations in the model formula such that theoretical values with the range -$\infty$ to $\infty$ are allowable, and a `normal(0, 2)` (location and scale) prior is used. For example in the `nec3param` model, $\beta = \text{beta}$ is an exponential decay parameter, which must by definition be bounded to 0 and $\infty$. Calling `exp(beta)` in the model formula ensures the exponent meets these requirements. Note also that a mean of 0 and standard deviation of 2 represents a relatively broad prior on this exponential scaling, so this is still a weakly informative prior in practice.
...
\pkg{bayesnec} adopts the weighting methods implemented in the \pkg{loo} package in R. **[WE NEED A PARAGRAPH HERE INTRODUCING LOO, THEN THE TWO MODEL AVERAGING METHODS IN STATISTICAL TERMS (I.E. WE'LL PROBABLY NEED SOME FORMULAS), AND TALK ABOUT THE LOO PACKAGE AND THE IMPLEMENTATION VIA BRMS AND RSTAN. The following sentence probably also more technical details]**. \pkg{loo} offers two weighting methods, the "stacking" method, aimed to minimise prediction error [@Yao2018], and the "pseudobma" method, with and without Bayesian bootstrap [@vehtari2020; @vehtari2017]. \pkg{bayesnec} currently uses by default the "pseudobma" method with Bayesian bootstrap, but this can be easily modified via argument "loo_controls".
...
```{r drccomparisonplot, echo = FALSE, fig.width = 6, fig.height = 3.5, fig.cap = "TBW."}
...
manecfit <- lapply(herbicides, FUN = function(x){
  ...
  fit <- bnec(fvfm ~ crf(concentration, model = "all"), data = data)
  return(fit)
})

all_plots <- lapply(manecfit, FUN = function(x){
  autoplot(x) +
...
```{r fullbayesmanecplot, echo = FALSE, fig.width = 6, fig.height = 8, fig.cap = "Full model averaged bayesmanecfits to seven phototoxicity datasets, showing estimated no effect concentrations. Data are the maximum effective quantum yield "}
library(ggpubr)
figure <- ggarrange(plotlist=all_plots, nrow=4, ncol=2, 
                    #labels = paste(letters[1:7], ". ",  herbicides, sep=""),
                    labels = herbicides,
                    vjust=3,
                    font.label=list(color="black",size=10, face = "plain"),
                    align = "hv")

annotate_figure(figure,
                left = text_grob("FvFm", rot = 90),
                bottom = text_grob("Concentration (natural log)"))
```
