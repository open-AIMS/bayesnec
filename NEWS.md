...
- `get_priors()` and `pull_prior()` now state that their output must be rebuilt
  ...
  extended formula. Adding `coef = "Intercept"` is not a general conversion
  because `brms` forbids coefficient-specific bounds (#381).
