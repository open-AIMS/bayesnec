...
manec_beta_identity <- nec_data %>%
  muted_bnec("x", "y", model = c("nec4param", "ecx4param"), chains = 2, family = Beta(link = "identity"))

message("\n# Binomial\n")
manec_binomial_logit <- nec_data %>%
  mutate(trials = 10, y = as.integer(round(y * trials))) %>%
  muted_bnec("x", "y", trials_var = "trials", model = c("nec4param", "ecx4param"), chains = 2)
...
manec_poisson_identity <- nec_data %>%
  ...
  muted_bnec("x", "y", model = c("nec4param", "ecx4param"),
             chains = 2, family = poisson(link = "identity"))

message("\n# Negative binomial\n")
manec_negbinomial_log <- nec_data %>%
  mutate(y = as.integer(round(exp(y * 3)))) %>%
  muted_bnec("x", "y", model = c("nec4param", "ecx4param"), chains = 2, family = negbinomial)
manec_negbinomial_identity <- nec_data %>%
  mutate(y = as.integer(round(exp(y * 3)))) %>%
  muted_bnec("x", "y", model = c("nec4param", "ecx4param"), chains = 2, family = negbinomial(link = "identity"))
...
manec_gamma_identity <- nec_data %>%
  mutate(y = exp(y * 3)) %>%
  muted_bnec("x", "y", model = c("nec4param", "ecx4param"), chains = 2, family = Gamma(link = "identity"))
