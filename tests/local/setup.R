...
manec_gausian_identity <- nec_data |>
  ...
  (\(x) {
    retry_mb(
      y ~ crf(x, model = c("nec4param", "ecx4param")),
      data = x,
      chains = 2
    )
  })()
...
manec_fits <- list(
  manec_gausian_identity = manec_gausian_identity,
...
mod_1 <- nec_data |>
  ...
  (\(x) {
    retry_mb(
      formula = y ~ crf(x, model = "nec4param"),
      data = x,
      chains = 2,
      n_mod = 1
    )
...
mod_2 <- nec_data |>
  ...
  (\(x) {
    retry_mb(
      formula = y ~ crf(x, model = "ecx4param"),
      data = x,
      chains = 2,
      n_mod = 1
    )
...
mod_3 <- nec_data |>
  ...
  (\(x) {
    retry_mb(
      formula = y ~ crf(x, model = "nechorme4"),
      data = x,
      chains = 2,
      n_mod = 1
    )
...
mod_4 <- nec_data |>
  ...
  (\(x) {
    retry_mb(
      formula = y ~ crf(log_x, model = "nechorme4"),
      data = x,
      chains = 2,
      n_mod = 1
    )
...
mod_5 <- nec_data |>
  ...
  (\(x) {
    retry_mb(
      formula = y ~ crf(log(x), model = "nechorme4"),
      data = x,
      chains = 2,
      n_mod = 1
    )
...
mod_6 <- nec_data |>
  (\(x) {
    retry_mb(
      formula = qlogis(y) ~ crf(x, model = "nechorme4"),
      data = x,
      chains = 2,
      n_mod = 1
    )
