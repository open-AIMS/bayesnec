...
fit1 <- bayesnec:::allot_class(fit1, "prebayesnecfit")
fit2 <- bayesnec:::allot_class(fit2, "prebayesnecfit")

test_that("expand_nec defaults work for nec model", {
  nec_fit <- bayesnec:::expand_nec(fit1)
  expect_equal(names(nec_fit), c("fit", "model", "inits", "pred_vals", "top",          
                                 "beta", "nec", "alpha", "bot", "d",            
                                 "slope", "ec50", "dispersion", "predicted_y", "residuals",    
                                 "nec_posterior"))
...
test_that("expand_nec arguments work for nec model", {
  nec_fit <- bayesnec:::expand_nec(fit1, x_range = c(0.01, 4), precision = 20)
  expect_equal(names(nec_fit), c("fit", "model", "inits", "pred_vals", "top",          
                                 "beta", "nec", "alpha", "bot", "d",            
                                 "slope", "ec50", "dispersion", "predicted_y", "residuals",    
                                 "nec_posterior"))
...
test_that("expand_ecx defaults work for ecx model", {
  ecx_fit <- bayesnec:::expand_nec(fit2)
...
test_that("expand_ecx arguments work for ecx model", {
  ecx_fit <- bayesnec:::expand_nec(fit2, x_range = c(0.01, 4), precision = 20)
  expect_equal(names(ecx_fit), c("fit", "model", "inits", "pred_vals", "top",          
                                 "beta", "nec", "alpha", "bot", "d",            
                                 "slope", "ec50", "dispersion", "predicted_y", "residuals",    
                                 "nec_posterior"))
...
test_that("expand_ecx sig_val argument work for ecx model", {
  ecx_fit_a <- bayesnec:::expand_nec(fit2)
  ecx_fit_b <- bayesnec:::expand_nec(fit2, sig_val = 0.1)  
  expect_gt(ecx_fit_a$nec["Estimate"], ecx_fit_b$nec["Estimate"]) 
})

tt1 <- manec_gausian_identity$mod_fits
test_null <- NULL
tt2 <- tt1["nec4param"]
