...
test_that("posterior passes correctly", {
  ae3 <- average_endpoints(list(ecx4param = ecx4param, nec4param = nec4param), posterior = TRUE)   
  expect_equal(length(ae3), 10)  
})

test_that("prob_vals passes correctly", {
  ae4 <- average_endpoints(list(ecx4param = ecx4param, nec4param = nec4param), prob_vals = c(0.3, 0.5, 0.7))   
