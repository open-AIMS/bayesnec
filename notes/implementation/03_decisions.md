...
| 4b | `bnec()` refuses a `resolution` below 2, before the model loop. It is a property of the call rather than of any one model, and reaching it from `expand_nec()` means reaching it after every model in the set has compiled and sampled --- the placement rule §11 records for `check_data()`. `check_args_newdata()` is the wrong place: `resolution = 1` is a valid request of `bnec_newdata()`, which `test-bayesmanec_methods.R` uses to pin a prediction shape |
...
`suc | trials(tot) ~ crf(log(dose_adj), model = "decline")`, 8000 draws. Of the draws `crossing_x()` discarded, every one reached
